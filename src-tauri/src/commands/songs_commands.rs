...
/// Stable error codes returned by `open_song_in_app`. Part of the IPC contract:
/// the frontend maps each one to its own localized toast, so do not reword them.
const SHELL_ERR_NOT_FOUND: &str = "not_found";
const SHELL_ERR_NO_ASSOC: &str = "no_assoc";
const SHELL_ERR_GENERIC: &str = "generic";
...
/// (and `explorer /select`, see above) drags in.
...
/// Open the file itself in whatever app Windows associates with its type —
/// unlike `open_song_in_explorer`, which only reveals it in a folder window.
#[tauri::command]
pub async fn open_song_in_app(path: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || shell_open(&path))
        .await
        .map_err(|e| e.to_string())?
}
