...
export function StreamItem({ stream, status, isFocused, isActiveRow, onPrimaryAction: _onPrimaryAction, onContextMenu: _onContextMenu, onDelete }: Props) {
  ...
  return (
    <li
      // The <li> itself is the 'summary' (whole-row) focus stop. It carries a
      // real `listitem` role, so aria-roledescription controls what NVDA reads
      // (e.g. "Mera, потік") instead of the bogus "section" that an aria-label
      // on a roleless <div> produced. The single whole-row focus ring comes from
      // the global [tabindex]:focus-visible rule in styles.css.
