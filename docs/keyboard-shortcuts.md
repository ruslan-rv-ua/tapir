...
- **Останнє звірення з кодом:** 2026-08-17 (search-focus-hotkey: Tier 2 — `Ctrl+F`
  (фокус у поле пошуку поточного екрана); та сама клавіша дописана в перелік гарда
  акселераторів, тож примітку «свідомо поза переліком: `Ctrl+F`» знято — її підстава
  («споживає Tier-2») хибна для відкритого діалогу, де Tier-2 виходить на `isInModal`);
  2026-08-14 (hotkeys-expansion: Tier 2 — `Ctrl+M`
...
4. **Дефолт мусить бути в межах токенів KeyRecorder** (літери, цифри,
   ...
   `codeToToken`) **і правила модифікаторів**: `Ctrl` або `Alt` обов'язковий
   (`Shift` сам по собі не модифікатор), голими законні лише `Pause` і
   `F13`–`F24` (`isBareLegal`; рішення —
   [key-recorder-bare-key](backlog/done/p1-key-recorder-bare-key.md)), інакше
   користувач не зможе його перезаписати.
...
`Pause` і `F13–F24` — єдині, що приймаються без модифікатора.
...
>
> `Ctrl+F` **не** гейтиться на секції — навпаки, вона єдина в Tier 2, що зобов'язана
> відповісти скрізь: конвенція обіцяє «знайти» на кожному екрані, а тиша на клавішу
> нерозрізненна з «застосунок завис». Ціль шукається не мапою «секція → зона», а
> першою зоною з методом `focusSearch?()` у `orderedZonesRef`
> ([useZoneNavigation.ts](../src/hooks/useZoneNavigation.ts)) — той список уже
> містить лише зони поточної секції, тож розсинхронитись немає з чим. Метод
> окремий від `focus`, бо `focus` зони — це `restoreFocus`
> ([useFocusBoundary.ts](../src/hooks/useFocusBoundary.ts)), тобто **останній
> елемент, якого торкались у зоні**: із сортування він на пошук не приведе.
> Клавіша ще й у переліку гарда (розділ нижче) — це другий шар, не дубль. Шари
> навмисно різної ширини: реєстр вимагає `!shiftKey` (`ctrlOrMeta`), гард —
> shift-агностичний за прецедентом `Ctrl+R`. Тобто `Ctrl+Shift+F` дефолт має
> погашений, а дії не має: `preventDefault` на комбінації без дефолту нічого не
> коштує, а вгадувати, які саме shift-варіанти WebView2 колись почне ловити, —
> та сама дірка, від якої стоїть родина `F5`.
>
...
| `Ctrl+F` · `Ctrl+Shift+F` · `Cmd+F` | Find bar WebView2 | ↑ (той самий предикат, що `Ctrl+R`) |
...
> «Копіювати / Перенести в профіль» (Tier 2′ вище), а `Ctrl+F` — Tier-2 фокус на
> пошук. Наслідок для KeyRecorder: `F5`, `Shift+F5` і `Ctrl+F` **резервуються** в
> `SHORTCUTS` (`row-copy-profile` / `row-move-profile` / `focus-search`), тож під
> OS-хоткей їх призначити не можна; `F3`/`F7`/`F11` лишаються вільними — у
> `SHORTCUTS` їх немає.
>
> **Два capture-слухачі на `window` не заважають один одному.** `stopPropagation()`
> у Tier-2 глушить лише подальші вузли, не інші слухачі того самого елемента (це
> робив би `stopImmediatePropagation`), тож `preventDefault()` гарда відпрацює
> незалежно від порядку реєстрації.
>
...
> до 2026-08-17** з підставою «споживає Tier-2» — вона хибна: Tier-2 виходить
> першим рядком на `isInModal()`, тож із будь-якого відкритого діалогу клавіша
> долітала до WebView2 і відкривала Find bar. Тепер вона в таблиці вище.
> Devtools у debug-збірці
