...
- **`F1` при наборі тексту:** під тим самим `shouldIgnoreShortcut()` (єдина спірна
  ...
  > **Уточнено (2026-06-08, KB-14):** це рішення переглянуто. Текстовий блок
  > прибрано з глобального слухача — він глушиться **лише** модаллю (`isInModal`),
  > як і `F6`. Тобто `F1`/`Alt+digit`/`Ctrl+K` тепер працюють і з фокусом у
  > текстовому полі (модифіковані/функційні комбо колізії з набором не дають).
  > `shouldIgnoreShortcut` прибрано. Деталі — KB-14 у бэклозі.
