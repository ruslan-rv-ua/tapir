...
main.rs → lib.rs::run()
  ...
  └─ tauri::Builder
       ...
       └─ .setup(|app| {
            ...
            │    --minimize → лише засув наміру; ховає frontend_ready (§5.3),
            │    коли документ уже є, — інакше вікно німе на весь сеанс
            │
...
| Гейт | Хто чекає | Чому |
|---|---|---|
| `frontend_ready` | план CLI, підсумок відновлення після збою, автовідтворення, **приховування вікна при старті згорнутим** | Webview підписується на події **після** первинного завантаження даних. Подія, надіслана раніше, зникає без сліду. Приховування тут із сусідньої причини: скрінрідер питає вікно про документ один раз, і сховане до завантаження вікно лишається німим на весь сеанс. Ховати — **першою дією** команди, поки гейт нижче не визнав вікно переднім |
...
| Помилки потоку | Автоперепідключення, а коли спроби вичерпано — стан `error` із причиною із закритого переліку (ADR 2026-09-06) |
...
- **Спроба зі стелею — одне значення в статусі.** Номер без стелі («спроба 3») не каже
  нічого, тож половини пари не існує й на дроті; писар один — `apply_transition`, і він
  же вирішує, що з переходу поїде в подію.
