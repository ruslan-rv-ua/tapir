...
| `update_stream` | `{streamId, name, url?, icyName?, bitrate?, format?}` | `StreamInfo` | Редагувати потік. Без `url` — чисте перейменування (`format`/`bitrate`/`icyName` недоторкані); з `url` — resolve PLS/M3U і перезапис усіх трьох похідних полів переданим, включно з `null`. Auth і per-stream ignorelist — беклог `p2-stream-auth.md`, `p2-per-stream-ignorelist-ui.md` |
