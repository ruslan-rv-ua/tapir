...
```jsonc
{
  ...
  // Згортати до трею замість закриття
  "minimizeToTray": true,
...
Окремий файл поряд із `settings.json` і `state.json`. Перелік комбінацій глобальних
гарячих клавіш, про зайнятість яких Tapir **уже сказав** людині (термін — CONTEXT.md
§«Гаряча клавіша, комбінація, зайнята комбінація»). Не налаштування (людина його не
редагує, у `settings.json` він виглядав би як поле, якого немає) і не сесійний стан
(переживає чистий вихід і аварію однаково; у `state.json` став би зобов'язанням для
писаря crash-recovery, який конструює файл заново).

```jsonc
{
  // Комбінації, репліку про зайнятість яких ДОСТАВЛЕНО — не просто виявлено.
  "combos": ["Ctrl+Shift+K"]
}
```
...
(`src-tauri/src/hotkey_busy.rs` — джерело правди. У TypeScript не віддається.)

### Семантика

| Аспект | Поведінка |
|---|---|
| Коли дописується | Лише після **доставленої** репліки: дренаж `BusyNotice` при першому показі вікна (старт) або команда `register_hotkeys` (вкладка «Гарячі клавіші» оголошує кожну нововиявлену комбінацію сама). Реєстрація як така нічого не додає — інакше старт згорнутим, після якого вікна не показали, зробив би конфлікт «повідомленим» назавжди. |
