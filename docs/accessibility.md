...
Доки [reconnect-counter-not-live](backlog/p2-reconnect-counter-not-live.md) не закрито,
код цього не виконує: цикл повертається в `connecting` на кожній спробі, а число в
сегменті застигле.
...
- **Кожна окрема невдала спроба** — ні репліки, ні тоста. Носій є в рядку («Спроба N з M»)
  ...
  **Носій мусить бути живим.** Мовчання виправдане тим, що число в рядку відповідає
  поточній спробі; поки воно застигле, мовчання не виправдане нічим. Це не теорія —
  див. [reconnect-counter-not-live](backlog/p2-reconnect-counter-not-live.md).
...
| Німе вікно після старту згорнутим (**чинне**) | Той самий закон, на крок далі: `--minimize` ховає вікно ще в `setup`, до завантаження документа. Скрінрідер питає вікно про документ **один раз**, застає порожнечу й більше не перепитує — ні на показ із трею, ні на зміну фокуса. Німим лишається весь застосунок, а не лише репліки | Тимчасово: перезапуск NVDA при відкритому вікні. Полагодити — ховати на `frontend_ready` ([minimized-start-silent-to-nvda](backlog/p0-minimized-start-silent-to-nvda.md)) |
