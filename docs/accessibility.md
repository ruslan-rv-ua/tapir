...
## 9. Діалог: Налаштування (Settings)

Повноекранний діалог, відкривається через ⚙️ кнопку внизу Activity Bar або Ctrl+,.
`role="dialog"`, `aria-label="Налаштування"`, focus trap, Escape — закрити.

Точки входу:
- ⚙️ gear (Activity Bar, над ProfileSwitcher)
- Ctrl+, (глобальний хоткей)
- Command Palette → "Налаштування"
- ProfileSwitcher → «Управляти профілями» (відкриває діалог, автоскрол до секції Профілі)

### 9.1. Структура

```
SettingsDialog (role="dialog", aria-label="Налаштування")
├── SettingsNav (role="navigation", aria-label="Категорії налаштувань")
│   └── Links: Загальні, Запис, Перепідключення, Гарячі клавіші, Постобробка, Аудіо, Профілі
│
├── SettingsContent (scrollable, headings correspond to nav links)
│
├── <h2> Загальні
│   ├── Select: Мова (aria-label)
│   ├── Select: Тема
│   ├── Checkbox: Згортати до tray
...
│
├── <h2> Запис
│   ├── TextField: Папка для записів (з кнопкою Browse)
│   ├── TextField: Шаблон імені файлу
│   │   └── Text (description): Прев'ю: "SomaFM Groove Salad/Tycho - A Walk" (динамічний preview)
│   ├── TextField: Шаблон неповного файлу
│   ├── TextField: Шаблон файлу потоку
│   ├── Checkbox: Зберігати файл потоку
│   ├── Checkbox: Пропускати перший неповний трек
│   ├── NumberField: Мін. тривалість треку (сек)
│   └── Checkbox: Автокорекція регістру
│
├── <h2> Перепідключення
│   ├── NumberField: Макс. спроб (0 = ∞)
│   ├── NumberField: Інтервал (сек)
│   ├── NumberField: Множник backoff
│   └── NumberField: Макс. інтервал (сек)
│
├── <h2> Гарячі клавіші
...
│   ├── TextField: Команда
│   ├── TextField: Аргументи
│   └── NumberField: Таймаут (сек)
│
├── <h2> Аудіо
│   └── Select: Пристрій виведення (з кнопкою Оновити)
│
├── <h2> Профілі
│   ├── Select: Активний профіль
│   ├── Button: Створити
│   ├── Button: Копіювати
│   ├── Button: Видалити
│   ├── Button: Експорт
...
`resumeFileFrom` — Settings → Audio → «Керування» ([data-models.md](data-models.md#1-глобальні-налаштування-datasettingsjson)).
