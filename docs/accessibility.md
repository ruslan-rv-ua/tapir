...
|---|---|
| `polite` | фоновий факт, який не відповідає на щойно натиснуту клавішу: старт/стоп запису, поразка потоку, збережений трек, зміна профілю, підсумки масових дій |
| `assertive` | відповідь на дію користувача (гучність, транспорт, F9), помилка цієї дії, збіг із wishlist, події планувальника |

**Вирішує близькість до дії, а не серйозність факту.** Слово «помилка» саме по собі
пріоритету не дає: `assertive` — це право обірвати диктора на півслові, і виправдовує
його лише те, що людина щойно натиснула клавішу й чекає відповіді. Поразка потоку —
контрприклад, на якому це правило вперше довелось прочитати вголос: вона настає через
≈40 хвилин після останньої дії руками, тож вона фоновий факт, хай і найнеприємніший
...
Ім'я несе **кожен** факт про рядок, а не найгучніший з них: стрілки вгору/вниз читають
саме його, сегменти лежать за Стрілкою вправо, а заливка й значки фаз від скрінрідера
сховані. Тому фаза запису звучить нарівні із самим записом — саме її скасовує друге
натискання «Записати» (запис `row-silent-while-connecting`).

| Стан рядка | Префікс імені | Плюс відтворення цього ж потоку |
|---|---|---|
| `recording` | «Записується» | «Записується і відтворюється» |
| `connecting` | «Підключення» | «Підключення і відтворення» |
| `reconnecting` | «Перепідключення» | «Перепідключення і відтворення» |
| `error` | «Помилка» | «Помилка і відтворення» |
| `idle` | префікса немає — саме тільки ім'я потоку | «Відтворюється» |

Три крапки, з якими фази стоять у сегменті («Підключення...»), в імені навмисно зняті:
там вони належать клітинці стану, а не промовленому імені.
...
| `reconnecting` | «Спроба {n} з {max}», або «Перепідключення...», якщо пари в статусі немає |
| `error` | причина відмови — «Станція не відповідає» або «Не вдалося записати на диск»: слово «Помилка» вже стоїть в імені рядка, і сегмент витрачається на те, чого імені бракує (ADR 2026-09-06 §5) |
| решта | «Очікування» |
...
не живе. Вони їдуть **однією величиною**, тож половини пари не буває: запасна гілка
лишається на випадок, коли пари немає зовсім.

`connecting` тут означає **перше** з'єднання запису. Спроби всередині перепідключення в
цей стан не повертаються, тож «Спроба N з M» стоїть у сегменті весь час боротьби, а не
лише в паузах між спробами
([ADR 2026-09-15](decisions/2026-09-15-connecting-is-the-first-connection.md)).
...
| Потік здався (спроби вичерпано, або диск відмовив) | announce + тост | `polite` | «{name}: Станція не відповідає» / «{name}: Не вдалося записати на диск» |
...
| Підключення (продовження останнього джерела) | announce | `assertive` | «Підключення — {name}» |
| Відновлення файлу з позиції | announce | `assertive` | «Відтворення — {name}, з {mm:ss}» |
| Останнє відтворення недоступне | announce; вікно не попереду — ще й **тост трею** (нативний) | `assertive` | «Останнє відтворення недоступне»; заголовок тосту — ім'я файлу або «Tapir» |
| Продовжити останнє не вдалося | announce; вікно не попереду — ще й **тост трею** (нативний) | `assertive` | «Помилка відтворення»; заголовок тосту — назва станції або ім'я файлу |
...
- **Кожна окрема невдала спроба** — ні репліки, ні тоста. Носій є в рядку («Спроба N з M»)
  і в метриці «Потребує уваги», яка рахує потік із першої невдачі; тост на кожну спробу
  давав десять спливань за ≈40 хвилин, жодного разу не сказавши головного. Звучить лише
  **поразка** — рядком вище. Свідома ціна: **перший обрив не звучить зовсім**
  ([ADR 2026-09-06](decisions/2026-09-06-error-is-the-diagnosis-attention-is-the-bucket.md) §4).

  **Носій мусить бути живим.** Мовчання виправдане тим, що число в рядку відповідає
  поточній спробі: застиглий носій не виправдовує нічого. Одного разу він таким і був —
  подія переходу пари не везла, і число оновлювалось лише на старті застосунку
...
| Продовжити останнє → стрім | «Підключення — <назва станції>» → «Відтворення: …» |
| Продовжити останнє → файл, `resumeFileFrom: position` (дефолт) і збережена позиція > 0 | «Відтворення — <назва треку>, з <mm:ss>» |
| Продовжити останнє → файл, `resumeFileFrom: start` (або збережена позиція 0) | «Відтворення: <назва треку>» (звичайний start-анонс, без позиції) |
...
`"resuming"` — тільки коли останнім джерелом продовжується файл із позиції:
озвучується **до** `play_file`, за тим самим патерном, що й `"connecting"` для стрімів
(де-ризик + one-shot suppression наступного дубльованого «Відтворення: …»). Перемикач
...
─────────
Грати / Пауза / Зупинити                             ← головний пункт; disabled, лише коли нічого не грає й останнього джерела немає
Зупинити                                             ← тільки для файлу (грає або на паузі)
─────────
...
| Грати / Пауза / Зупинити | "Грати", "Пауза" або "Зупинити" | Головний пункт — та сама роль, що й головна кнопка плеєра й `Ctrl+Shift+K` (CONTEXT.md §«Головна кнопка і останнє джерело»): живе джерело — «Зупинити», файл — «Пауза» / «Грати», нічого не грає — «Грати», що продовжує останнє джерело. Вимкнений, лише коли останнього джерела немає: сірий пункт — видимий носій межі |
| Зупинити | "Зупинити" | Тільки для файлу: живе джерело головний пункт уже зупиняє, а два пункти з тим самим словом скрінрідер не розрізнить |
...
| `BackgroundFeedback` | фоновий відгук — відповідь на дію у фоні, зроблену людиною або виконану за її налаштуванням | **без гейта** — це єдиний слід дії у фоні, а не ефірна балаканина |
...
| Німе вікно після старту згорнутим | Той самий закон, на крок далі: `--minimize` ховав вікно ще в `setup`, до завантаження документа. Скрінрідер питає вікно про документ **один раз**, застає порожнечу й більше не перепитує — ні на показ із трею, ні на зміну фокуса; заговорити змушував лише перезапуск NVDA | Ховати вікно на `frontend_ready`, коли документ уже є. Ціна — вікно видно до секунди при старті згорнутим ([minimized-start-silent-to-nvda](backlog/done/p0-minimized-start-silent-to-nvda.md)) |
