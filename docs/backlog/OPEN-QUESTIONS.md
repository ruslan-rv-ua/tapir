...
### [command-palette-phase-3](p2-command-palette-phase-3.md) (✅ фіналізовано 2026-07-23 → planned/ready)
- ✅ **Станції вирізано з фази** — пункти-потоки цілком у `command-palette-taxonomy`; phase-3 = пісні + навігація, per-stream дії не чіпаються.
- ✅ **Ліміт результатів:** порожній query → статус-кво (дії + per-stream) + навігація в кінці; пісні лише при query ≥ 2 символів, до 10 збігів (найновіші за `recordedAt`).
- ✅ **Пошук пісень:** metadata Є — backend повертає повний `Song` (artist/title/album/station/…); haystack = fileName+artist+title+album+station, токенізований substring (fuzzy — окремий запис `command-palette-fuzzy-search`).
- ✅ **Навігаційні команди:** видимі одразу при порожньому рядку; генеруються з реєстру `SECTIONS` (вкл. «Профілі») + «Налаштування»; роутера немає — через `$activeSection`/`$settingsDialogOpen`.
- ✅ **Взаємодія з Phase 4:** `PaletteItem.type: "action"|"song"|"navigate"` закладено; порядок фіксований, ранжування — phase-4.
- ✅ **Кнопка «Команди» (DA5):** питання зняте — кнопок у коді вже немає (Ф1/Ф2 фактично виконані), палітру відкриває лише Ctrl+K.
...
### [import-duplicate-metadata-update](p3-import-duplicate-metadata-update.md) (ready, рішення прийняті 2026-07-23)
- ✅ **Що оновлювати:** усі probe-поля (`name`/`format`/`bitrate`/`icy_*`). `update_stream` редагує лише `name`, тож ручні правки інших полів неможливі; diff назви видно в підписі чекбокса. (Поля `codec` не існує — у коді це `format`.)
- ✅ **Коли probe:** зондувати дублікати разом з усіма при відкритті діалогу (зараз фільтр їх виключає — стану «probe вже виконано» для дублікатів не існувало).
- ✅ **Грануляція:** один чекбокс на рядок, показується коли будь-яке поле відрізняється; підпис — diff назви або перелік змінених полів.
- ✅ **Undo:** без undo — послідовно з rename/delete; захист = явний opt-in чекбокс з diff.
- ✅ **Нормалізація:** case-insensitive + trim; спека ICY `icy-name` не нормалізує (довільний текст мовника) — прикладне рішення.
