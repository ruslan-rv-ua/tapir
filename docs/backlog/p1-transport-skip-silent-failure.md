...
- [ ] Поверхню обирає фокус вікна: у фокусі — `addToast`, поза — нативний тост `HotkeyFeedback`
- [ ] Повідомлення називає **ціль** і причину з двох ключів; сирих рядків через межу процесів немає
- [ ] `seek-start` із глобальної клавіші оголошується; нативного тоста в нього немає
- [ ] `SkipHooks` містить лише `beforeExecute`; німим третій викликач стати не може
- [ ] `announce()` є функцією в `src/stores/announcer.ts`, `useAnnounce` — обгортка над нею
- [ ] Нова команда вузька й типізована; `keys!` має два нові варіанти, `messages/` — нуль нових рядків
- [ ] Новий ADR «Три поверхні відповіді: вухо, вікно, система» написано й перехресно злінковано з обох наявних ADR і `AGENTS.md`
- [ ] `CONTEXT.md` §«Оголошення, сповіщення і видимий носій» має четвертий пункт і посилання на новий ADR
- [ ] `docs/help/{uk,en}/player.md` оновлено; `troubleshooting.md` і `background.md` не чіпані
- [x] Заведено окремий p2-запис — [transport-boundary-silent-in-background](p2-transport-boundary-silent-in-background.md)
- [ ] NVDA-прогін пройдено — два стани вікна на два емітери, плюс перевірка тиші на межі
