...
> **Аудит (2026-06-15):** Повний CLI реалізовано. **`--datadir` відхилено** — занадто складно (потрібно змінювати `portable.rs`, глобальний mutex обмежує цінність) і невідома реальна потреба. Фіча вважається завершеною. End-to-end ручне тестування включає: 20 unit-тестів `cli.rs`, ручний запуск всіх аргументів.
...
| ~~`--datadir PATH`~~ | ~~Перевизначити директорію даних~~ ❌ відхилено |
...
**Обмеження:** `--profile` має сенс лише при старті першої інстанції. При форвардингу (друга інстанція → перша) ігнорується з попередженням і NVDA-оголошенням. `--datadir` відхилено.
...
- [x] `--record NAME|URL` запускає запис (за назвою профілю або URL)
- [x] `--play NAME|URL` запускає відтворення
- [x] `--stop-recording` зупиняє всі активні записи
- [x] `--stop-playback` зупиняє відтворення
- [x] `--wish-add PATTERN` / `--wish-remove PATTERN` керують wishlist і оголошуються через NVDA
- [x] `--profile NAME` вибирає профіль при запуску (session-only, не зберігається в settings.json)
- [x] `--minimize` запускає застосунок у tray (без видимого вікна)
- ~~[ ] `--datadir PATH` перевизначає директорію даних~~ **відхилено**
- [x] При повторному запуску argv форвардиться першій інстанції і виконується
- [x] `--profile` при форвардингу → polite NVDA-оголошення про ігнорування
- [x] Exit codes: 0 (success / --help / --version), 1 (runtime error), 2 (invalid args)
- [x] Помилки (stream not found, invalid URL, action failed) → assertive NVDA + error-toast
- [x] `--help` і `--version` → exit 0, без відкриття вікна
- [x] 20 unit-тестів `cli.rs` + ручне end-to-end тестування пройдено
- [x] `pnpm test` і `cargo test` проходять

## Прийняті рішення

| Питання | Рішення |
|---------|--------|
| `--datadir` і `portable.rs`? | ❌ **Відхилено.** Занадто складно, глобальний mutex обмежує цінність, реальна потреба не виявлена. |
| `--datadir` при форвардингу? | N/A (відхилено). |
...
- Код: `src-tauri/src/portable.rs` — `data_dir()`
