...
| [reconnect-counter-not-live](p2-reconnect-counter-not-live.md) | P2 | planned | ready | M | [reconnect-max-in-status](done/p2-reconnect-max-in-status.md) ✅ | [recording-duration-is-per-connection](p3-recording-duration-is-per-connection.md) | пара «спроба N з M» не їде в події переходу, тож у рядок потрапляє лише на старті застосунку; грумінг 2026-09-15 закрито, два ADR |
