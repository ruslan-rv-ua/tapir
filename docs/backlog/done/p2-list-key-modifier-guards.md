...
summary: "ADR: клавіші списку голі, поки не названо; `suppressesDefault` похідний від `resolveKeyAction`; з `Alt`/`Meta` не гасимо — `Alt+Space` вільний."
priority: P2
type: planned
status: done
effort: S
...
Дефолт клавіатури в списках розвернуто: `switch` у `resolveKeyAction` метчить лише **голі**
клавіші, а комбінація, яку список справді має, оголошена гілкою **вище** одного спільного гарда
(`Ctrl+Space`, `Ctrl+C`, `Ctrl+A`, `Shift+↑/↓`, `Enter`, `Tab`/`Shift+Tab`, `F5`/`Shift+F5`).
Власні гарди `F2`/`F4` прибрані як зайві. Правило й відхилені варіанти — [ADR «клавіші списку
голі, поки не названо інакше»](../../decisions/2026-09-04-list-keys-are-bare-unless-named.md).
Спадок на майбутнє — **три речі**. `Enter` бере рівно **один** модифікатор із `{Shift, Ctrl,
Alt}`: пара більше не розв'язується порядком `if`-ів (три списки мали три різні таблиці
пріоритету), а `AltGr+Enter` = `Ctrl+Alt` перестав віддавати потік зовнішній програмі. Вердикт
«погасити дефолт» дає чистий предикат `suppressesDefault`, **похідний** від `resolveKeyAction`
(«а голою вона б спрацювала?») — власного переліку клавіш він не має навмисно, тож нова клавіша
в `switch` потрапляє під гард сама; ціна тієї ж монети — клавіша, яка **мусить** везти
модифікатор, мовчатиме, поки її не названо. І межа гасіння: `preventDefault()` без
`stopPropagation()` лише коли модифікатори суто `Ctrl`/`Shift`, бо з `Alt`/`Meta` це шар ОС —
саме звідси єдине **набуття** запису, `Alt+Space` знову відкриває системне меню вікна Windows
(NVDA-прогін підтвердив: WebView2 його не з'їдає). Виняток із гасіння один — `Enter`/`Space` під
нативною кнопкою (кнопка дії, «Завантажити ще») лишаються браузеру; навігаційні клавіші винятку
не мають. Знахідка, якої в записі не було: `Shift+Space` у потоках **слухав** усупереч докам —
`StreamList` віддавав `mods` у `activateStream` із клавіатурної гілки `toggle`, а тест писали на
`Alt+Space`, єдиному модифікаторі, якого той не читав (зелений тест над хибним твердженням).
Свідомо не чіпали: `Meta` лишився псевдонімом `Ctrl` у трьох названих винятках (`ctrlOrMeta` —
конвенція усього застосунку), тож критерій про `Meta` тримається на **шляху відмови**. Борг,
названий явно: `Shift+Home`/`End`/`Page*` стали інертні —
[list-shift-range-to-edge](../p2-list-shift-range-to-edge.md). **NVDA-прогін проведено
2026-09-04, усі 4 сценарії пройдено, зауважень немає**
