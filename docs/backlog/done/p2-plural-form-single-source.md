...
summary: "`src/lib/plural.ts::plural` — єдине джерело форми множини; локаль лише з `getLocale()`, бо він обирає текст; хелпер бере набір форм, не суфікс."
priority: P2
type: planned
status: done
effort: S
...
Шість `Intl.PluralRules` із трьома різними входами (`document.documentElement.lang`,
`settings?.language`, `getLocale()`) зведені в `src/lib/plural.ts` — `plural(count, forms)`,
локаль **тільки** з `getLocale()`, параметра `locale` немає свідомо: саме `getLocale()` обирає
**текст**, і форма мусить читати ту саму змінну, інакше в одному реченні два незалежні входи.
Хелпер бере **набір форм-танків**, а не готовий суфікс: `crash_resume_all` має лише
`_one/_few/_many`, `profile_stream_count` — ще й справжній `_other`, і суфіксний хелпер
ламається рівно на цій парі; побічний виграш замикання — підстановки понад `{count}` (`{label}`,
`{names}`) заходять без окремої обгортки, через яку `StreamsPanel` тримав власний `pluralize`.
Запасний хід `other → _many` живе **в одному місці** з коментарем, чому ключ зветься так (CLDR
для `en` дає `one/other`, але конвенція спільна з Rust-шаром і старша за CLDR), а нуль лишився
випадком **застосунку**: `count === 0` перевіряється до `Intl.PluralRules` і лише там, де ключ
`_zero` існує. Головна знахідка: **сайтів шість, а викликів одинадцять** — таблиця запису
рахувала конструктори правил, а `StreamsPanel` ховав за одним конструктором шість викликів
локальної обгортки, три з них у `useCallback`; грепом за `Intl.PluralRules` — тобто власним
критерієм запису — їх не видно, знайшов їх `pnpm typecheck`. Ту саму ваду того самого дня,
паралельно й на кілька хвилин раніше, закрив точковий запис
[crash-resume-plural-english-rules](p2-crash-resume-plural-english-rules.md): дві гілки дійшли
до `getLocale()` незалежно, при злитті взято версію через хелпер, а тест лишився **його** —
сильніший, бо пришпилює `<html lang="en">` у `beforeEach` на всі випадки й мокає `getLocale()`
замість покладатись на `baseLocale`. Рівно як записи домовились наперед: хто йде першим, той і
лишає тест. Урок про вказівники: перейменування зламало **три** посилання на стару конвенцію
(два в ADR native-layer-localisation і док-коментар `plural_suffix` у `i18n.rs`), і жодного не
стереже тест: `docsLinks` перевіряє існування файлу, а не рядок, а імені функції в коментарі не
перевіряє ніщо. Це й єдиний відступ від критеріїв — `i18n.rs` мав лишитись недоторканим, і
торкнулись його рівно на один коментар
