...
summary: "`Ctrl+F` іде в першу зону з `ZoneEntry.focusSearch?()`; без пошуку — assertive-репліка; `KeyF` у гарді акселераторів, бо з діалогу відкривав Find bar."
priority: P2
type: planned
status: done
...
> з переліку гарда в [webview-reload-guard](p2-webview-reload-guard.md) §8.
...
- [x] NVDA-прогін (мануально, перед релізом): приземлення фокуса на обох екранах,
      репліка на секції без пошуку, `Ctrl+F` з відкритого діалогу — тиша без Find bar.
      **Проведено 2026-08-17 за чеклістом `docs/testing/nvda-search-focus-hotkey.md`
      (8 сценаріїв), усі пройдено, зауважень немає; чекліст видалено на прийманні.**
...
`Ctrl+F` веде фокус у поле пошуку поточного екрана. Спадок — **опційний метод на контракті
зони**: `ZoneEntry.focusSearch?()`
([useZoneNavigation.ts](../../../src/hooks/useZoneNavigation.ts)) реалізують лише зони з
текстовим пошуком, а клавіша бере **першу таку** з `orderedZonesRef`. Мапи «секція → зона» немає
свідомо: той список уже містить лише зони поточної секції плюс постійні, тож розсинхронитись
немає з чим, і майбутньому пошуку потоків ([streams-list-search](../p2-streams-list-search.md),
відкрите питання про клавішу там закрито) вистачить реалізувати метод — репліка на Потоках
зникне сама. Чому не `focus` зони: він дорівнює `restoreFocus`
([useFocusBoundary.ts](../../../src/hooks/useFocusBoundary.ts)), тобто **останній торканий
елемент** — із сортування на пошук він не привів би. Фокус уже в полі → `select()` (спільний
`lib/focusOrSelect.ts`, бо тіло було байт-у-байт однакове у двох компонентах). Клавіша
**глобальна без `when`-гейта**: на чотирьох екранах без пошуку йде assertive-репліка «На цьому
екрані немає пошуку» — тиша у відповідь на конвенцію, обіцяну скрізь, нерозрізненна з
«застосунок завис». Другий шар — **гард акселераторів**: `KeyF` приєднано до `KeyR` у
[webviewAccelerators.ts](../../../src/lib/webviewAccelerators.ts), бо Tier-2 виходить першим
рядком на `isInModal()`, тож із будь-якого відкритого діалогу клавіша долітала до WebView2 і
відкривала Find bar. Цим **спростовано підставу**, з якою `Ctrl+F` виключили з переліку в
[webview-reload-guard](p2-webview-reload-guard.md) §8 («споживає Tier-2») — примітку в реєстрі
виправлено, а не просто прибрано. Шари навмисно різної ширини: реєстр вимагає `!shiftKey`, гард
shift-агностичний, тож `Ctrl+Shift+F` має погашений дефолт і не має дії — `preventDefault` на
комбінації без дефолту безкоштовний, а вгадувати shift-варіанти WebView2 — та сама дірка, від
якої стоїть родина `F5`. Знахідка код-рев'ю, полагоджена там же: критерій «фокус лендиться з
будь-якої зони секції» тримався на висновку з двох окремих half-тестів (синтетичні зони проти
прямого виклику `focusSearch`) — додано наскрізний тест на обох панелях, де справжні зони
склеєні зі справжнім слухачем, як у `App.tsx`. **NVDA-прогін проведено 2026-08-17, усі 8
