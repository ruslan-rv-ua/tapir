...
summary: "`0` = не перепідключатися, стеля 10000 клампиться і в `Profile::load`; спроба скидається на першому аудіобайті; одна `plan_retry` замість трьох."
priority: P0
type: planned
status: done
effort: M
...
Знахідка grilling `help-recording` (розбіжність «0» між `manager.rs`, підказкою й
`StreamItem.tsx`) сама пройшла окрему grilling-сесію (12 питань) перед реалізацією й виросла з
`S` до `M`: другий шар помилки — `attempt = 0` скидався на успішному `connect`, а не на першому
аудіобайті, тож станція, що приймає з'єднання й одразу рве потік, ніколи не впиралась у стелю
спроб. Ухвалено: `0` = «не перепідключатися» (не «необмежено» — цього стану в домені більше
нема), дефолт 10 спроб, стеля 10000 клампиться на бекенді. Три копії правила «пробувати ще раз?»
в `manager.rs` звелися в одну `plan_retry`. Рішення й відхилені альтернативи — в нового ADR
[reconnect-attempt-semantics](../../decisions/2026-08-13-reconnect-attempt-semantics.md);
словник отримав розділ «Перепідключення і спроба». **Код-рев'ю після реалізації знайшло дві
реальні прогалини**, обидві виправлено в тому ж коміті: кламп стелі 10000 був підключений лише
до збереження налаштувань (`apply_settings_patch`), не до `Profile::load`/`save_imported` —
профіль, збережений до появи стелі (або зібраний вручну), відновлював би нескінченність чорним
ходом; і на шляху read-error `plan_retry` рахував повний план (з `compute_backoff_delay`) двічі
на ту саму спробу — з нього виділено дешевий `would_retry` без backoff-обчислення. Третя
знахідка рев'ю — свідомо не виправлена тут: `StreamItem` читає живі `maxRetries`, а бекенд
працює зі знімком налаштувань на старті запису, тож під час активного перепідключення вони
можуть розійтися; це вже відщеплений і розблокований
[reconnect-max-in-status](p2-reconnect-max-in-status.md). Наявні профілі не мігруються
(пре-реліз) — старий `maxRetries: 0` лишається, поки користувач не пересохранить налаштування
вручну
