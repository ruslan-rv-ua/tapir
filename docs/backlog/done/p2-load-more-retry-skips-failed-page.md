...
summary: "ADR loaded-prefix-is-the-cursor: offset = `$searchResults.length` у мить запиту; чужа порція мусить відхиляти проміс, не тихо виходити."
priority: P2
type: planned
status: done
effort: S
...
Позиція пагінації більше **не зберігається**: offset — це довжина того, що вже на екрані
(`$searchResults.length`), і рахується в мить запиту, тож невдалій порції нема чого псувати, а
повторне натискання просить ту саму сторінку. `$searchParams` став `SearchCriteria =
Omit<SearchParams, "offset">` (дріт у `tauri.ts` не чіпали — межа по шару, не по полю),
`isAppendingResults` зник разом зі своїм читачем, і слухач курсора в `BrowserPanel` скидає
курсор **безумовно**: кожна зміна критеріїв тепер і справді нова вибірка. Рішення — [ADR
loaded-prefix-is-the-cursor](../../decisions/2026-09-04-loaded-prefix-is-the-cursor.md). Спадок
на майбутнє — **квиток порції**: `loadMore` тримає посилання на об'єкт критеріїв і на
приземленні звіряє його з поточним; будь-яке місце, що «оновить» критерії новим об'єктом із тими
самими значеннями, викине порцію в польоті. І пастка, яку знайшло рев'ю: чужа порція мусить
