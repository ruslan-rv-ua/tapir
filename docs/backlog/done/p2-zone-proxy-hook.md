...
summary: "Дев'ять проксі → `useZoneProxy(id, ref)`, що повертає сам `ZoneEntry`, не реф: об'єкт один на екземпляр, лежить у deps ефекту без перезапуску."
priority: P2
type: planned
status: done
effort: S
...
Дев'ять однакових проксі в шести файлах — один хук `useZoneProxy(id, ref)` поруч із `ZoneEntry`.
Рішення, яке варто знати: хук повертає **сам `ZoneEntry`, а не реф до нього** — об'єкт
створюється раз на екземпляр, тож він лежить у залежностях ефекту реєстрації без жодного
перезапуску, а наявні директиви `exhaustive-deps` і далі глушать лише `onZonesChange`, як і
кажуть їхні коментарі. Пояснення «навіщо проксі» тепер має одного власника, і саме через це
рев'ю спіймало його першу помилку: коментар посилав по історію Songs-рескану в
[wishlist-stale-list-ref](p1-wishlist-stale-list-ref.md), а той запис про **інший** баг
(callback-ref, занулений exit-анімацією TabPanel); справжня історія — коміт e00262a. Урок про
чеклісти: коли перевірка мусить назвати, **де стоїть фокус після дії**, вона знаходить місця, де
він не стоїть ніде — Enter на «Скинути фільтр» демонтує власну кнопку і кидає фокус на `<body>`,
наступний `F6` починає коло з бокової панелі; це існувало до запису й відкрито як
[streams-reset-filter-focus-drop](p2-streams-reset-filter-focus-drop.md). Друге, що дав чекліст:
випадок «той самий лік — новий хендл», заради якого проксі існує, руками відтворюється лише на
одному екрані — два широкі пошуки в Браузері станцій, обидва зрізані по 50. Метод рев'ю, який
варто повторювати: кожен процитований рядок звірено 1:1 з `uk.json`, порядок зон у чеклісті — з
ефектами реєстрації. **NVDA-прогін проведено 2026-09-05, усі 7 сценаріїв пройдено, зауважень
немає**
