...
summary: "Стеля їде в `StreamStatus.reconnect_max_retries` парою з `reconnect_attempt` з одного знімка; єдиний писар `mark_reconnecting` під тестом."
priority: P2
type: planned
status: done
effort: S
...
«Спроба N з M» у рядку потоку перестала брати N і M з двох джерел: стеля тепер їде в
`StreamStatus.reconnect_max_retries` (`reconnectMaxRetries`) поруч із `reconnect_attempt`, з
того самого знімка налаштувань, за яким живе цикл `'reconnect`, а `StreamItem` читає обидва
числа зі статусу — проп `maxRetries` зник, `StreamList` більше не підписаний на
`$profileSettings`. Інваріант «обидва або жодне» тримає єдиний писар `mark_reconnecting`,
виділений у чисту функцію (бо `StreamManager` без `AppHandle` не збирається) і закритий тестом;
гілку `maxRetries > 0` в UI прибрано — стан `reconnecting` зі стелею `0` при спільному знімку
неможливий, лишився один запасний варіант «Перепідключення…» на відсутню половину пари. Сторожі
спереду — два тести `StreamList`: стеля видна до завантаження профільних налаштувань і не
змінюється, коли профіль каже інше число. Довідка без змін — `recording.md` уже обіцяє «номер
