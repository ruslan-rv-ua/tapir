...
summary: "Одне ім'я з обох боків IPC — `isLiveSource()` і `PlaybackSource::is_live()`, тотальні таблиці під тестом; третього бейджа для прев'ю немає."
priority: P2
type: planned
status: done
effort: M
...
Плеєр перестав пропонувати паузу живому звуку — і пауза та справді спрацьовувала:
`pause_playback` джерела не перевіряв, rodio ставав на паузу, станція йшла далі, буфер протухав.
Корінь — предикат `type === 'stream'`, що відповідав на «це потік?», коли питання було «це живий
звук?»; жив у чотирьох місцях, два з них уже були праві (`decide_toggle`, `seek_playback`).
Питання дістало **одне ім'я з кожного боку IPC**: `isLiveSource()` у `src/lib/playbackSource.ts`
і `PlaybackSource::is_live()` у Rust. Обидва **тотальні** — таблиця по всіх видах джерела, не
`!== "file"`: заперечення тихо зарахувало б майбутній вид у живі, а компілятор тут не сторож
(`vite:build` не кличе `tsc`), тож сторожем зроблено тест, який називає всі види поіменно. Рядок
бітрейту став рядком живого джерела: бейдж **LIVE** завжди, бітрейт лише коли є `StreamInfo`,
прочерка немає — «—» означає «ще не прийшло», а тут «не буває». Третього бейджа для прев'ю **не
заводили**: керування тотожне ефіру, а те, що станцію ще не додано, несе кнопка рядка в
каталозі. SMTC `Pause` перейшов на `is_live()` — інакше дефект переїхав би з екрана на апаратну
клавішу. `F9` втратив гілку прев'ю разом із ключем `f9_preview`: прибиралась не різниця (ключі
були дослівно рівні), а **можливість** завести її правкою локалі — без видимого носія на екрані.
ADR свідомо немає: третій бейдж додається будь-коли одним компонентом, модель несуть CONTEXT.md
§«Живе джерело» і сам запис
