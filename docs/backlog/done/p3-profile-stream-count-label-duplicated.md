...
summary: "Власник «{count} потоків» один — `profileStreamCount.ts`; опис запису брехав: бейдж рядка рендерить увесь `{countLabel}`, голе число — лише діалог."
priority: P3
type: planned
status: done
effort: S
...
Власник рядка «{count} потоків» тепер один — `src/lib/profileStreamCount.ts`; обидва компоненти
імпортують його, жодного символа вихідного рядка не змінено, моки двох тестів-сторожів не
правлені взагалі. **Головна знахідка — опис запису брехав про власну підставу**: «на обох
поверхнях видно лише голе число» — голе число видно тільки в діалозі, а `aria-hidden`-бейдж у
рядку профілю рендерить **увесь** `{countLabel}` зі словом. Інваріант від цього міцнішає, не
слабшає: слово «потоки» на одній поверхні — видимий текст, на другій — лише доступна назва, тож
розходження копій було б водночас видимим в одному місці й чутним в іншому. Другий урок — про
**межу між тестом хелпера й тестом його клієнта**: випадок `en → other:2` здається копією
`plural.test.ts`, але той пришпилює диспетчеризацію при наявній формі, а цей — що родина взагалі
подає четверту форму; зняття `other` з модуля перевірено — червоніє рівно він один, бо `uk` до
цієї категорії не доходить
