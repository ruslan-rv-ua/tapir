---
slug: crash-resume-plural-english-rules
title: "Відновлення після аварії озвучується англійськими правилами множини"
summary: "Форму множини диктує `getLocale()`, як і текст; jsdom лишає `lang` порожнім, тож тест був зелений даремно — виставляй `lang` у `beforeEach`."
priority: P2
type: planned
status: done
effort: S
kind: bug
target: 0.1.0
updated: 2026-09-05
completed: 2026-09-05
a11y: false
depends_on: []
blocks: []
touches:
  - src/hooks/useCrashResumeFeedback.ts
gates: [pnpm test, pnpm typecheck, pnpm vite:build]
notes:
  - "Знахідка дослідження paraglide-native-plurals (2026-09-04), перевірена в коді на HEAD."
  - "index.html віддає lang=en, App.tsx:146 перезаписує його ПІСЛЯ резолву getSettings(); useMemo із deps: [] фіксує англійські правила назавжди."
  - "a11y: false свідомо — ані фокус, ані мовчання не зачіпаються, змінюється лише граматична форма; текст іде і в тост, тож перевіряється очима й тестом, без NVDA-прогону."
---

# Відновлення після аварії озвучується англійськими правилами множини

> **Опис нижче — стан ДО виправлення** (2026-09-04): він цитує знятий `useMemo`, і
> рядкові якорі на `useCrashResumeFeedback.ts` ведуть у файл тієї редакції, не в
> теперішній. Що стоїть там зараз — «Виправлення».
>
> **Контекст:** знахідка попутно в дослідженні
> [paraglide-native-plurals](p2-paraglide-native-plurals.md). Механізм перевірено
> в коді на HEAD, не з опису. Вада жива для української локалі.

## Опис

[useCrashResumeFeedback.ts:18](../../../src/hooks/useCrashResumeFeedback.ts:18) створює
правила множини так:

```ts
const pluralRules = useMemo(
  () => new Intl.PluralRules(document.documentElement.lang || "uk"),
  [],
);
```

Три обставини складаються в неправильну форму:

1. [index.html](../../../index.html) віддає `<html lang="en">`, тож `|| "uk"` **ніколи** не
   бере праву гілку — запасне значення тут декоративне.
2. Атрибут перезаписує лише [App.tsx:146](../../../src/App.tsx:146), і то **після** того,
   як зарезолвиться `getSettings()`.
3. Хук кличеться на першому рендері `App` ([App.tsx:435](../../../src/App.tsx:435)), а
   `deps: []` фіксує створені тоді правила **на всю сесію**.

Результат: українцеві `select(3)` дає `other`, ланцюжок тернарників
([useCrashResumeFeedback.ts:28](../../../src/hooks/useCrashResumeFeedback.ts:28)) падає в
`_many`, і замість «Відновлено 3 записи» звучить і показується «Відновлено 3 записів».
Те саме на 2, 4, 22 — тобто на всій категорії `few`.

Сусіди від цього застраховані: `StatusBar` створює правила щорендеру (після резолву
налаштувань читає вже `uk`), `StreamsPanel` бере `settings?.language` першим,
`SongsPanel` монтується після завантаження налаштувань. Ламається саме той шлях, що
стартує найраніше.

## Виправлення

Локаль брати з `getLocale()` — тим самим джерелом, яким `m.*()` обирає **текст**, а не з
атрибута `<html>`, який на момент першого рендеру ще англійський.

Структурно це та сама зміна, яку робить
[plural-form-single-source](p2-plural-form-single-source.md) для всіх шести місць. Записи
свідомо роздільні: вада має закритися незалежно від того, коли дійдуть руки до хелпера.
Хто йде першим, той і лишає тест; другий підбирає його під себе.

## Критерії готовності

- [x] `docs/help/` — запис видимої поведінки не змінює (текст і так мав бути правильний)
- [x] Регресійний тест: подія `crash-resume` з `resumed: 3, total: 3` при українській
      локалі дає форму `_few`; перевірено, що на невиправленому коді він **падає**
- [x] `document.documentElement.lang` більше не є входом у цьому файлі
- [x] Тост і `announce()` несуть той самий рядок (як зараз)

## Спадок

Форму множини в `useCrashResumeFeedback` тепер диктує `getLocale()` — те саме джерело, що обирає
**текст**; `document.documentElement.lang` із файлу пішов, а `useMemo` знято зовсім: правила
будуються в обробнику події, бо мову міняють без перезавантаження (`GeneralTab`: `setLocale(…, {
reload: false })`), і `deps: []` заморозили б її вдруге. Найцінніше — **урок про тест**: наявний
тест був зелений лише тому, що jsdom лишає `lang` порожнім, тож мертве запасне `|| "uk"` там
**спрацьовувало**, а в застосунку ні. Перший крок виправлення — не код, а рядок
`document.documentElement.lang = "en"` у `beforeEach`, як його віддає `index.html`; після нього
впав і новий регресійний випадок (3 із 3 → `all-many-3` замість `all-few-3`, дослівно симптом із
запису), і **старий** випадок на 2 з 2, який доти нічого не стеріг. Тест на мок `getLocale`, а
не на атрибут, — щоб він пережив перехід на спільний хелпер. Структурне закриття класу приїхало
того самого дня, наступним записом —
[plural-form-single-source](p2-plural-form-single-source.md); при злитті двох гілок узято його
хелпер, а тест форми для цього хука лишився звідси. Прийнято без чекліста NVDA (`a11y: false`):
ні фокус, ні мовчання не зачіпаються — змінюється граматична форма рядка, що йде і в
`announce()`, і в тост

## Документи

- [paraglide-native-plurals](p2-paraglide-native-plurals.md) — звідки знахідка, з розбором усіх шести місць
- [plural-form-single-source](p2-plural-form-single-source.md) — структурне закриття того самого класу
