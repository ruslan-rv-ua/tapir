...
depends_on: []
blocks: []
touches: [src-tauri/src/autostart.rs, src-tauri/src/settings.rs, src-tauri/src/lib.rs, src-tauri/src/commands/settings_commands.rs, src/components/settings/GeneralTab.tsx]
gates: [cargo test, cargo clippy, pnpm test, pnpm vite:build]
depends_on_external: ["Phase 2C (SettingsDialog, ✅)", "Phase 3A (System Tray, ✅)", "Phase 3D (Scheduler, ✅)"]
---

# Autostart (Підфаза 3I-2)

> **Контекст:** виконано повністю (бекенд + frontend + тести), перевірено в коді 2026-06-25. Механізм — прямий запис у `HKCU\...\Run` через `winreg`, не `tauri-plugin-autostart`.
