...
summary: "Запис існує від команди «почати», підключення — його фаза; умова одна — `recordingToggle.ts`; відмови `already_recording`/`not_recording` ковтаються."
priority: P1
type: planned
status: done
effort: M
...
Чотири місця — кнопка рядка, контекстне меню, Enter у списку, палітра — вирішували «почати чи
зупинити» власним `state === "recording"`, тож усі ≈40 хвилин перепідключення пропонували почати
вже живий запис і доносили англійську відмову бекенду до тосту. Правило, заради якого варто
читати запис, — у словнику: **запис існує від команди «почати»**, підключення й перепідключення
— його фази, тому дія рядка каже «Зупинити запис» у трьох фазах, як уже казала глобальна кнопка.
Одна умова дістала один дім — `recordingToggle.ts`; відмови «вже пишеться» і «нема чого
зупиняти» їдуть кодами `already_recording`/`not_recording` і **ковтаються**, бо «вже пишеться» в
застосунку тричі трактовано як пропуск. **Відхилено** підписи по фазах (сфокусована кнопка
базікала б на кожному переході) та ідемпотентний старт (у вікні між «зупинити» й прибиранням
запис ще є, але вмирає). Заливка рядка розводить фази — жовта для підключення, червона для
байтів, — а кнопка лишається червоною: її колір кодує дію, не фазу. **Знайдено під час
реалізації:** тест менеджера на нові варіанти `RadioError` неможливий без `AppHandle` — гарантію
