...
summary: "`ZoneId` — 21 літерал, `data-zone-id` типізовано аугментацією `HTMLAttributes`: одруківка валить `pnpm typecheck`; хендлам явний `(): ZoneEntry =>`."
priority: P2
type: planned
status: done
effort: S
...
Союз `ZoneId` на 21 літерал плюс аугментація `HTMLAttributes` у `useZoneNavigation.ts`:
одруківка в будь-якому з 67 місць тепер падає на `pnpm typecheck` — перевірено **сімома
підсадженими одруківками одразу**, tsc назвав усі сім файлів, п'ять із «Did you mean». Знахідка,
що зняла сам `draft`: TS **перевіряє оголошений `data-*` атрибут** — `declare module "react" {
interface HTMLAttributes<T> { "data-zone-id"?: ZoneId } }` звіряє літерал на кожному
intrinsic-елементі й відхиляє `string`-змінну, чим примусово перевело `ScreenZone.id` і
`CompositeList.zoneId` на союз; неоголошені `data-*` лишились вільними. Дві пастки в спадок:
`useImperativeHandle(ref, () => ({ id: "…" }))` **розширює літерал до `string`** ще до перевірки
`R extends T`, і помилка тоді стоїть на цілому об'єкті без імені зони — п'ять хендлів дістали
явний `(): ZoneEntry =>`, пояснення один раз у док-коментарі `ZoneId`; а ESLint `no-unused-vars`
свариться на `<T>`, який declaration merging мусить повторити дослівно, — єдина директива, з
причиною в тексті. Попутно знято мертву опцію `zoneId` в `useCompositeList` («reserved… Task 4»,
ніхто не читав) і подвоєний літерал у `StationList` (константа `satisfies ZoneId` живить і
пропс, і селектор). Термін: елемент із `data-zone-id` — **контейнер зони**, не «носій» (той
зайнятий глосарієм під стан на екрані; у `CONTEXT.md` не пішов — DOM-деталь). Тест існування
контейнерів відхилено свідомо: брехав би на порожніх станах і на проксі. Порядок із сусіднім
хвостом розвернувся сам: [zone-proxy-hook](p2-zone-proxy-hook.md) злився першим,
`useZoneProxy(id)` перетипізовано в мерж-коміті. Прийнято розробником без чекліста NVDA — діф
продуктивного коду лише типи, одна константа й знята опція; підстава в записі
