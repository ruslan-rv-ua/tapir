...
summary: "Знято чотири крейти й npm-пакет; пряма залежність буває другою копією крейта; зняття міняє features сусідів — правду каже діф `Cargo.lock`."
priority: P2
type: planned
status: done
effort: S
...
Чотири крейти й один npm-пакет, що компілювались нізащо, прибрано: `stream-download` (жодного
`stream_download` у `src`), `unicode-normalization` (умову зняв ICY-запис, який видалив єдиний
`.nfc()`), `cpal` і `futures`, плюс deprecated `@inlang/paraglide-vite`. Два уроки на майбутнє.
**Перший: пряма залежність буває не зайвим рядком, а другою копією крейта.** У lock жили `cpal
0.15.3` (наша) і `cpal 0.17.1` (rodio), а код звертався лише до другої через `rodio::cpal` —
разом із дублікатом пішли й аудіо-бекенди `alsa`/`coreaudio`/`ndk`/`oboe` для платформ, яких ми
не постачаємо. Тому критерій «версія в lock не змінилась» читається як «версія, якою
користується код». **Другий: зняття крейта тихо змінює features сусідів.** `stream-download`
своїм `reqwest-rustls` був єдиним, хто вмикав у `reqwest` HTTP/2, тож із дерева пішов `h2` і
застосунок ходить у мережу лише по HTTP/1.1 — TLS цілий (`rustls` оголошуємо самі), ALPN просто
не пропонує `h2`; у діфі `Cargo.toml` цього не видно взагалі, знайшлось лише в діфі lock-файлу.
Із пари `futures`/`futures-util` лишився **`futures-util`**, усупереч опису запису: він і так
транзитивний через reqwest, tokio-util, hyper-util і tower, тоді як парасолька тягне ще й
`futures-executor`; переїхали п'ять звернень у трьох командних модулях, а `stream/connection.rs`
лишився недоторканим. Заразом `architecture.md` перестав вимагати Unicode NFC «(crate
`unicode-normalization`)» у санітизації імен файлів і показувати `raw.nfc()` у прикладі, який
після зняття крейта не скомпілювався б — `sanitize_component` NFC ніколи й не робив. Обсяг:
`Cargo.lock` −395 рядків, `pnpm install` −119 пакетів. Ручний прогін 9/9
