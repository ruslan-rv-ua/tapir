...
summary: "«Ні» обом: `wildmatch` і `use-debounce` не заводяться — свій код малий і без відомих багів; тригер — справжній баг; `usehooks-ts` губить правку."
priority: P3
type: planned
status: done
effort: S
...
Рішення «**ні**» по обох замінах: власний DP `wildcard_match` у `matcher.rs` і `useAutoSave`
лишаються як є, `wildmatch` і `use-debounce` не заводяться. Підстава — код малий, покритий
тестами й без відомих багів; заміна прибирає те, що не ламалось, і додає залежність, за якою
треба стежити. Запис закрито саме для того, щоб наступний аудит «велосипедів» не відкривав
питання знову; тригер повернення — справжній баг в одному з двох шматків. Побічна знахідка на
майбутнє: `usehooks-ts` як заміна `useAutoSave` не годиться — на розмонтуванні робить `cancel`,
і остання правка налаштувань губиться
