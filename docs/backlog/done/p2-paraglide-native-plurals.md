...
summary: "Не мігрувати: `i18n::parse` кладе Rust на масиві будь-де, гілки paraglide позиційні, а `serde_json` віддає `count=0` останнім; обрано один хелпер."
priority: P2
type: research
status: done
effort: M
...
Дослідження, яке закінчилось **відмовою мігрувати** — і трьома поправками до власного опису.
Варіанти Paraglide вміють усе, що треба: точний збіг `count=0` поруч із категоріями CLDR працює
вже на встановленій **2.15.3** (бамп до 2.25.0 для задачі не потрібен), і формат живе в тих
самих файлах — змінюється лише тип значення, рядок стає масивом. Ламає це **другого споживача**:
`i18n::parse` десеріалізує **цілий файл** у `HashMap<String, String>`, тож масив **будь-де**
кладе Rust — байдуже, читає він цей ключ чи ні. Тому варіант (б) «мігрувати лише те, чого Rust
не читає» **неможливий** у своєму формулюванні: підмножини родин без правки `i18n.rs` не існує.
У варіанті (а) знайшлась пастка, якої запис не бачив: **порядок гілок у paraglide позиційний**
(повний catchall друкується безумовним `return`, тож поставлений першим робить решту мертвим
кодом — мовчки), а `serde_json` без `preserve_order` — це `BTreeMap`, який віддасть Rust
`count=*, countPlural=*` **першим**, а `count=0` **останнім**. Наївний порт «перший збіг виграє»
був би не складнішим, а **тихо неправильним**. Обрано **(в)**: один хелпер на `getLocale()`,
JSON і `cargo test` не чіпаємо — бо болять не суфікси, а шість копій вибору з двома незалежними
входами в одне речення (форму обирає `document.documentElement.lang`, текст — `getLocale()`).
Три поправки до опису запису: сайтів **шість**, не п'ять (`StreamTransferDialog.tsx` не було
навіть у `touches:`); запасне `|| "uk"` — **мертвий код**, бо `index.html` віддає `<html
lang="en">`, тобто реальний дефолт англійський; справжніх родин множини **одинадцять**, не
чотирнадцять — `browser_probe_failed`, `profile_switch_scheduled` і
`tray_quit_confirm_scheduled` гілкуються за `len() == 1` і мають **різні** підстановки в гілках,
тож варіанти звели б їх в один тип входу, де обов'язкові чужі поля: для цих трьох міграція —
регрес типів, а не виграш. З мертвого запасного значення виросла **жива вада**
([crash-resume-plural-english-rules](p2-crash-resume-plural-english-rules.md)). Метод, який
варто повторювати: варіантну форму не звіряли з документацією — її **скомпілював і виконав** той
самий компілятор із `node_modules`, а порядок ключів `serde_json` перевірено окремим крейтом.
Наслідок — [plural-form-single-source](p2-plural-form-single-source.md)
