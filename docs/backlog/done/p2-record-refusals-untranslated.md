...
summary: "Відмови запису й плеєра йдуть кодами без чисел: числа мають видимий носій, деталь у `warn!`; один `reportRecordRefusal` замість трьох `catch`."
priority: P2
type: planned
status: done
effort: S
...
- **`play_stream` і його «stream not found: …».** Той самий клас і, після цього запису,
  ...
  візьметься, ключ перевикористовує, не заводить другий. **Закрито того ж дня**, одразу
  після приймання: `PLAY_ERR_STREAM_NOT_FOUND` — аліас константи запису, один `case` у
  `playRefusalMessage`, ключ той самий; шлях гарячих клавіш правки не потребував, бо
  `reportSkipFailure` уже відповідає локалізовано.
...
Хвіст record-action-lies-while-connecting: на `start_recording` лишались дві англійські відмови
— за диском з числами і «Stream <id> not found», — і перша була досяжна з **шести** місць, не
чотирьох, бо «Записати все» / «Записати виділені» ганяють ту саму перевірку першою. Спадок —
застосування ADR 2026-09-06 §5 до відмови, а не поразки: **код без чисел**, бо числа вже мають
видимий носій (рядок стану й метрика «Вільно» перемикаються на «Мало вільного місця» за тим
самим порогом), а деталь лишається в логу — `check_disk_space` уперше пише `warn!` при відмові,
бо доти числа жили лише в тості. Grooming знайшов, що з чотирьох споживачів перевірки прозу
показували лише дві команди — планувальник, crash-recovery і CLI її вже логували, — тож Display
варіанта `DiskSpaceLow` лишився англійським для логу, а на дріт іде код. **Зниклий потік — тост,
не тиша**: «вже пишеться» ковтається, бо тричі трактоване як пропуск, а рядка, якого немає в
профілі, список не мав показувати, і тиша сховала б саме цю ваду. **Знайдено під час
реалізації:** відповідь на відмову стала одним `reportRecordRefusal` для одиночного старту й
обох «Записати все» замість трьох `catch` із власним текстом. Прийнято без NVDA-прогону (`a11y:
false`): нових зупинок фокуса й live-поверхонь немає. Хвіст — та сама проза в `play_stream` —
закрито того ж дня: константа плеєра є аліасом константи запису, ключ той самий
