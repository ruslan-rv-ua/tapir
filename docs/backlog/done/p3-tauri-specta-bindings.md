...
summary: "Не мігрувати, поки `tauri-specta` в RC: експортер відмовляє на `u64`/`usize` і `deserialize_with`; гілку `spike/tauri-specta` не зливати."
priority: P3
type: research
status: done
effort: L
...
Дослідження зі спайком, яке **відхилило** міграцію на `tauri-specta`, поки крейт у RC: остання
`2.0.0-rc.25` (2026-05), docs.rs для двох останніх версій не збирається, питання про вихід із RC
без відповіді, rc.24 змінив форму згенерованих типів. Тригер повернення — стабільний `2.x` із
зеленим docs.rs або **друга** справжня помилка дрейфу. Спадок — нотатка
[tauri-specta-bindings.md](../../notes/tauri-specta-bindings.md) (32 джерела, три роди тверджень
позначено явно) і гілка `spike/tauri-specta`, яку не зливають. Головний урок — **ціна не там, де
її бачив запис**: генерація за мілісекунди, `vi.mock` живий, але експортер відмовляє на 21 полі
`u64`/`usize`, рендерить `f32` як `number | null`, відмовляє на `deserialize_with` і робить `?:`
з кожного `#[serde(default)]` у DTO-результатах — це переформатування типової поверхні
фронтенду, не «мінус 925 рядків»; повна міграція ≈ 5–7 днів, «лише типи» на стабільному ts-rs ≈
2–3. Друге: обіцянка запису про `vi.mock` стосувалась **одного** файлу з 99 — решта 36 мокають
саму обгортку `lib/tauri`, яку міграція мала б прибрати. Аудит 67 ручних типів проти Rust
знайшов 12 розбіжностей, справжня одна — `track-changed` із плеєра не несе `ignored`; вона стала
записом [tauri-ts-type-drift](p2-tauri-ts-type-drift.md). Пастка Windows на майбутнє: lib-тест,
що згадує `Builder::<Wry>`, не стартує (`STATUS_ENTRYPOINT_NOT_FOUND`) — tauri-build вшиває
маніфест Common-Controls v6 лише в bin-цілі, а `rfd` тягне `TaskDialogIndirect`; обхід —
`rustc-link-arg-tests` у `build.rs`, і лише для `[[test]]`-цілі
