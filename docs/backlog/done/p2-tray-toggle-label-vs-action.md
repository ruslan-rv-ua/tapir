...
summary: "Живому звуку в треї лише «Зупинити»: `MenuPlayback` не виразить «на паузі й живе»; `Ctrl+Shift+K` файл не зупиняє — глобальної зупинки немає."
priority: P2
type: planned
status: done
effort: S
...
Меню значка перестало обіцяти паузу там, де дія зупиняє: для живого звуку — і ефіру потоку, і
станції з каталогу — головний пункт зветься **«Зупинити»**, а окрема «Зупинити» поруч не
малюється взагалі. Розвилку запису («прибрати окрему / прибрати перемикач / розвести тексти»)
грилінг звузив до одного варіанта знахідкою, яку варто пам'ятати: **для живого джерела обидва
пункти були однією командою** — `persist_session_snapshot` для живого нічого не робить, і обидва
шляхи кінчались у `player.stop_playback`, тож розводити тексти було нічим. Спадок —
`MenuPlayback { Idle, Live, FilePlaying, FilePaused }` у `tray/mod.rs`: знімок меню більше не
може виразити «на паузі й живе», а чиста `playback_items` тримає під тестом усе, що `build_menu`
міг би переплутати (сам він бере `AppHandle` і юніт-тестом не береться). Дві пастки на майбутнє.
**Порядок армів у `from_status` компілятор не стереже** — арм із `is_live()` має охоронця, тож
підняти над ним `Playing` можна, і ефір знову дістане «Пауза» з зеленими гейтами; сторож —
`from_status_names_every_source_kind`, перевірений мутацією. І **`Ctrl+Shift+K` не зупиняє
файл** — для файлу це пауза/продовження, а глобальної клавіші зупинки відтворення в Tapir немає
взагалі (`stop_all` = `Ctrl+Shift+S` зупиняє записи); це ледь не коштувало хибного `[!]` в
NVDA-чеклісті. Заразом виправлено id пункту в `docs/architecture.md`, де решта секції трея
лишається ескізом дореалізаційних часів.
