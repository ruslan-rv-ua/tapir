...
summary: "ESLint 10, область як у `pnpm typecheck`; несучий — прапорець `--max-warnings 0`, бо обидва правила — `warn`; `linterOptions` без `files` ловить усе."
priority: P2
type: planned
status: done
effort: S
...
ESLint 10, плаский конфіг, область та сама, що в `pnpm typecheck` (`src` і `build`). Несучий тут
**прапорець, а не пресет**: `pnpm lint` = `eslint --max-warnings 0`, бо обидва правила, заради
яких усе робилось (`exhaustive-deps` і мертва директива), мають рівень `warn` — без прапорця
ворота не падали б ніколи. Ворота перевірені підсадженим порушенням кожного з трьох класів, як і
clippy. Дві пастки плаского конфігу, обидві спіймані першим прогоном: блок `linterOptions` **без
`files`** застосовується до всіх файлів і цим втягнув у перевірку 544 згенеровані `.js`
paraglide, а покинутий каталог `src/paraglide/` (git його не відстежує) довелось ігнорувати
окремо від робочого `src/i18n/paraglide/`. `no-unused-vars` вирівняно з tsconfig по підкресленню
— різниця лише в `caughtErrors`, яких TypeScript не бачить узагалі, і саме там знайшлись **дві
проковтнуті помилки** аудіопристрою (`AudioTab.tsx`): причина відмови не доходила ні до тоста,
ні до логу. Попутно: мертва директива прибрана, ніколи не придушене порушення в
`usePlayerZoneNav.ts:93` закрите, а `SongTagsUpdatedPayload` із порожнього інтерфейсу став
псевдонімом **і почав використовуватись** там, де подію набирали `Song` вручну. Єдине придушення
на весь проєкт лишене свідомо й із поясненням — до
[zone-entry-dead-el](p2-zone-entry-dead-el.md)
