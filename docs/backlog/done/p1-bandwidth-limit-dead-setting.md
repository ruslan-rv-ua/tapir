...
summary: "`bandwidthLimitKbps` прибрано: рішення вже було у відхиленій фазі 3I-4; сторож: старий `settings.json` вантажиться, ключ назад не пишеться."
priority: P1
type: planned
status: done
effort: S
...
Перший запис теми «мертві налаштування прибрані»: `bandwidthLimitKbps` жив у `settings.json` і в
документації, але жодного споживача в Rust не мав — `architecture.md` описував тротлінг у
`StreamManager`, якого ніколи не існувало. Рішення виявилось **уже ухваленим ззовні**: підфазу
3I-4 відхилено 2026-06-15 (радіо-бітрейт 128–320 kbps і так малий, кейсу насичення каналу
немає), а поле пережило відхилену фазу — запис лише виконав другу половину того рішення. Спадок
— **дві звички, не код**: перша — перед грилінгом «реалізувати чи прибрати?» шукати, чи рішення
вже не записане у фазах; друга — форма сторожа при дорелізному видаленні поля, успадкована від
[profile-scoped-settings](p0-profile-scoped-settings.md): один тест перевіряє **обидві**
половини сумісності — що старий `settings.json` із зайвим ключем завантажується (serde ігнорує
невідомі поля) і що ключ не пишеться назад. Побічно виправлено опис самого запису: UI-редактора
поле не мало ніколи, «рубильник» існував лише на диску — нагадування, що `touches:` у беклозі це
оцінка, а не факт.
