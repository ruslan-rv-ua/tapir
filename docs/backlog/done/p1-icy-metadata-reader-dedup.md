...
summary: "Розбір ефіру один — `IcyMetadataReader` і `pump_air`/`AirEvent`; читач добиває буфер, буфер не менший за `metaint`; `AirTap` дає межу треку."
priority: P1
type: planned
status: done
effort: M
...
Розбір ефіру став один на застосунок: два ручні цикли `metaint`, дві копії `ReqwestSyncReader` і
три копії розбору `StreamTitle` замінено на `IcyMetadataReader` із крейта, який уже був у
залежностях. Заразом зникло обрізання назви на першому апострофі («Don't Stop» → «Don»);
натомість з'явилась рідша вада того ж роду — крейт ріже поле по `;`, тож назва з крапкою з комою
обірветься на ній. Спадок — **насос ефіру**: `connection::pump_air` + `AirEvent`, а плеєр і
рекордер лише мапять подію у свою; тіло відповіді (`IcyConnection.response`) стало приватним,
тож `prefix` фізично не можна віддати не першим. **Три пастки, кожна під тестом у
`stream/connection.rs`.** Перша: читач мусить **добивати буфер до кінця** — крейт вважає, що
прочитане дорівнює замовленому, і після короткого читання (а `bytes_stream` віддає шматками
мережі) шукав би байт довжини метаданих посеред аудіо. Друга, знайдена вже тестом на межу: крейт
ріже `buf[..next_metadata]`, а `next_metadata` доростає назад до `metaint`, тож **буфер, менший
за `metaint`, виходить за межі — на короткому читанні, тобто рівно на обриві**; станція з
`metaint: 16000` (SomaFM, найтиповіше значення) роняла б так кожен запис. Звідси правило «буфер
не менший за `metaint`», а `metaint` понад 1 МіБ відхиляється, щоб заголовок не диктував розмір
виділення; обидва числа приходять з `into_reader` разом, бо саме їх розходження й повернуло б
падіння. Третя, не про крейт: **callback кличеться із середини `read`**, тобто буфер на той
момент містить і хвіст старого треку, і початок нового — без поправки назва випереджала б свій
звук на цілий буфер, і рекордер різав би файл на пів секунди зарано (це знайшло рев'ю, не
тести); `AirTap` рахує зняті з тіла байти й дає межу. Невалідний UTF-8 лишився паритетним —
`from_utf8_lossy`, **NFC не додано**, тож `unicode-normalization` тепер мертвий остаточно й іде
до [dead-dependencies](p2-dead-dependencies.md). Scaffold-и
`decode_icy_metadata`/`parse_stream_title` видалено разом із `allow(dead_code)` — виняток із
правила «scaffold не чіпати» дав сам запис. Попутно переписано `architecture.md` §5.1.1, який
показував `decode_icy_metadata` з latin-1 і NFC як чинний алгоритм. **Ручний прогін на живому
ефірі проведено 2026-09-04** (SomaFM, `metaint: 16000`): та сама пара в плеєрі й рекордері, межа
треку на місці, обрив без падіння
