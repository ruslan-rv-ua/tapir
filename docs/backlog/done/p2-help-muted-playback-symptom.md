---
slug: help-muted-playback-symptom
title: "Довідка: симптом «відтворення йде, а звуку немає» — вимкнений звук"
summary: "Порада «Tapir скаже» мусить називати видиму поверхню, бо `announce()` — `sr-only`; передача читача сусідній позиції — лише симптом → причини."
priority: P2
type: planned
status: done
effort: S
kind: chore
target: 0.1.0
updated: 2026-08-16
completed: 2026-08-16
a11y: false
depends_on: [hotkeys-expansion, help-troubleshooting]
blocks: []
touches:
  - docs/help/uk/troubleshooting.md
  - docs/help/en/troubleshooting.md
  - docs/backlog/done/p1-help-content-polish.md
gates: [pnpm test, pnpm vite:build]
notes:
  - "Знахідка код-рев'ю hotkeys-expansion (2026-08-14): прогалина існувала й до нього — розділ «Якщо щось не працює» має дві причини тиші з трьох. Ctrl+M лише підняв імовірність третьої."
  - "a11y: false за прецедентом усіх шести записів довідки (help-troubleshooting): текст, який читають, а не поведінка, яку слухають."
  - "2026-08-16 (grilling): дію позиції змінено з F9 на пару «кнопка плеєра + F9». Причина — F9 віддає відповідь ЛИШЕ в sr-only (useGlobalShortcuts.ts:49 → LiveAnnouncer.tsx:56); тоста немає, рядок стану не змінюється. Для читача без скрінрідера чернетка радила дію, яка не робить нічого видимого, чим порушувала принцип універсального дизайну зі спеки. Кнопка — єдина поверхня, що працює для всіх."
  - "2026-08-16 (там же): Ctrl+M як самостійна перевірка відкинута. Її зворотний зв'язок — сам звук, а у відмовному напрямку звуку вже немає; оголошення теж sr-only. Наосліп вона може мовчки вимкнути звук тому, у кого причина була інша, і не показати цього."
  - "2026-08-16 (там же): нульова гучність лишається поза цим записом, але вже не як відмова — її звели з mute у коді, запис zero-volume-reads-as-muted. Причина в позиції справді одна."
  - "2026-08-16 (там же): позиція вводить у розділ нову конвенцію — передачу читача сусідній позиції. Жодна з 13 наявних цього не робить; ціна прийнята свідомо, бо це єдине місце, де спека будує ланцюг («перша у своїй трійці»), а найзагальніший заголовок стоїть перед двома вужчими."
---

# Довідка: симптом «відтворення йде, а звуку немає» — вимкнений звук

> **Контекст:** одна позиція у вбудованому розділі «Якщо щось не працює»
> ([troubleshooting.md](../../help/uk/troubleshooting.md)). Специфікація довідки —
> [help-content-polish](p1-help-content-polish.md) (читати перед правкою),
> сам розділ завела [help-troubleshooting](p1-help-troubleshooting.md).
> Форму позиції перевизначив grilling 2026-08-16 — див. «Що написати».

## Опис

Розділ «Якщо щось не працює» знає **дві** причини тиші при справному відтворенні —
не той пристрій виведення й від'єднаний пристрій — і не знає **третьої**, найдешевшої
в перевірці: звук просто вимкнено.

Симптом важчий за решту рівно тому, що ззовні все справне: рядок грає, позиція
рухається, індикатор не червоний. Стан вимкненого звуку в застосунку видно **в одному
місці** — кнопка плеєра змінює лейбл на «Увімкнути звук» і має `aria-pressed`.

[hotkeys-expansion](p2-hotkeys-expansion.md) прогалину не створив, але зробив її
ймовірнішою: `Ctrl+M` тепер працює з будь-якого місця вікна, тож випадкове натискання
дістало шлях, якого раніше не було — кнопка вимагала походу в зону плеєра.

## Що написати

Одна позиція `###` у `troubleshooting.md`, обидві локалі. Форма — за
[help-content-polish](p1-help-content-polish.md): заголовок словами користувача
(не назвою підсистеми), **причина + дія, 1–3 речення**, без переказу механіки.

**Місце в порядку — між позиціями 6 і 7** («Станція записується, але не відтворюється»
→ **тут** → «Звук іде не в той пристрій»). Правило розділу — порядок за стадією, на
якій симптом помічають; усі три належать до «що чути», а ця перевіряється найдешевше,
тож іде першою у своїй трійці.

Чернетка uk (текст остаточно шліфувати при реалізації):

> ### Відтворення йде, а звуку немає
>
> Найпростіша причина — звук вимкнено. Стан показує кнопка в панелі плеєра, а `F9`
> називає його з будь-якого місця вікна; повертає звук `Ctrl+M`. Якщо звук увімкнено,
> причина, найпевніше, у пристрої виведення.

### Три рішення, вбудовані в ці фрази

**1. Кнопка названа поруч із `F9`, а не замість неї.** `F9` віддає відповідь виключно
в `sr-only`-регіон ([useGlobalShortcuts.ts:49](../../../src/hooks/useGlobalShortcuts.ts:49)
→ [LiveAnnouncer.tsx:56](../../../src/components/common/LiveAnnouncer.tsx:56)): тост не
піднімається, рядок стану не змінюється. Порадити саму `F9` означало б написати текст,
який має сенс лише для користувача скрінрідера, — заборонено принципом універсального
дизайну. Кнопка плеєра — єдина поверхня, спостережувана всіма; обидва шляхи в одному
реченні — це та сама вимога спеки.

**2. `Ctrl+M` стоїть як дія повернення, а не як перевірка.** Її зворотний зв'язок — сам
звук, і у відмовному напрямку його вже немає, а оголошення теж `sr-only`. Порада
«просто натисніть» наосліп вимикає звук тому, у кого причина була інша, і не показує
цього.

**3. Останнє півречення передає читача далі.** Заголовок позиції — найзагальніший у
розділі й єдиний у своїй трійці, сформульований як **симптом**: сусідні два названі
причинами («Звук іде не в той пристрій», «Звук зник після від'єднання пристрою»), яких
читач ще не знає. Без передачі загальний заголовок перехоплював би й тих, чия причина
в 7 або 8, і закінчувався б глухо.

### Чого в позиції свідомо немає

- **«`Ctrl+M` діє з будь-якого місця вікна, тож натиснути її випадково легко»** —
  переказ [player.md:15](../../help/uk/player.md:15), тобто чуже пояснення, плюс оцінка,
  з якої читачеві нічого робити. До того ж теза вужча, ніж звучить: `Ctrl+M` вимагає
  сфокусованого вікна, а [muteCleanup.ts](../../../src/lib/muteCleanup.ts) сам знімає mute
  на зупинці й на зміні джерела — натиснути мусили під час цього самого відтворення.
- **Відсилання «докладніше — у розділі «Плеєр і відтворення»»** — жанр дозволяє його
  лише там, де без механіки дія неповна. Тут дія повна.

Два наслідки для сусідніх текстів:

1. Позиції 7 і 8 лишаються як є — але тепер читач доходить до них після найдешевшої
   перевірки й **за прямою вказівкою**, а не випадково.
2. Мапа покриття в [help-content-polish](p1-help-content-polish.md) отримує
   14-ту позицію, а рядок «Ціна визнана: 13 заголовків проти 3–6 у решті розділів»
   стає «14».

## Критерії готовності

- [x] `docs/help/uk/troubleshooting.md` і `docs/help/en/troubleshooting.md`: нова
      позиція між 6 і 7, формула «причина + дія», без таблиць і посилань
- [x] Позиція називає **і** кнопку плеєра, **і** `F9` — жоден із двох шляхів не
      єдиний (принцип універсального дизайну)
- [x] Позиція закінчується передачею до причини «пристрій виведення»
- [x] Стеля розділу не перевищена (200–1000 слів по en-файлу, правило 6 довідки;
      en-файл до правки — 846 слів)
- [x] Мапа `troubleshooting.md` у [help-content-polish](p1-help-content-polish.md):
      14-та позиція + лічильник заголовків
- [x] `pnpm test` (парність локалей — `helpContent.test.ts`), `pnpm vite:build` зелений

## Поза скоупом

- **Показувати вимкнений звук поза кнопкою плеєра** (значок у рядку стану, тост,
  суфікс у заголовку вікна) — це зміна поведінки, а не довідки. Заведено окремо:
  [sound-hotkeys-feedback-announce-only](p1-sound-hotkeys-feedback-announce-only.md).
  Пряме питання того запису — не «чи вистачає `F9`», а «чому глобальна клавіша не дає
  нічого видимого».
- **Нульова гучність.** Не другою причиною в тексті, а зведенням її з вимкненим звуком
  у коді: [zero-volume-reads-as-muted](p2-zero-volume-reads-as-muted.md). До того запису
  позиція лишається правдивою, але покриває лише перемикач; після нього ті самі три
  мітки починають працювати й для нульового рівня. Залежності немає свідомо — текст під
  обидва стани однаковий.
- **Решта причин тиші, які лікує сам застосунок** (буферизація на старті потоку) — у
  розділі свідомо немає позицій для того, що минає само.
- **`AAC+`-станції, які не відтворюються** — уже позиція 6, окрема причина й окрема
  дія.

## Спадок

Чотирнадцята позиція `troubleshooting.md` — «Відтворення йде, а звуку немає», сьома в порядку,
перша у трійці «що чути». Спадок ширший за позицію й лежить у двох правилах для майбутніх
текстів. **Перше:** `LiveAnnouncer` — `sr-only`, тож `announce()` не має видимої поверхні
взагалі; будь-яка порада «натисніть клавішу, і Tapir скаже» є текстом лише для частини читачів і
мусить називати видиму поверхню поруч (тут — кнопку плеєра). Записано в мапу
[help-content-polish](p1-help-content-polish.md); чинний рядок такого класу лишився в
`player.md:33` і чекає на
[sound-hotkeys-feedback-announce-only](p1-sound-hotkeys-feedback-announce-only.md). **Друге:**
розділ дістав конвенцію передачі читача сусідній позиції — дозволена рівно там, де
заголовок-симптом стоїть перед заголовками-причинами того ж симптому, як загальне правило не
поширюється. Свідомо відхилено: `Ctrl+M` як перевірка (її відгук є сам звук, якого в цьому
симптомі немає) і дослівна назва підпису кнопки (перемикається за станом, тож у «здорового»
читача такої кнопки на екрані немає). Друга знахідка сесії — нульова гучність дає ту саму тишу,
липкіше й непоміченою: [zero-volume-reads-as-muted](p2-zero-volume-reads-as-muted.md) +
[ADR](../../decisions/2026-08-16-silence-is-mute-or-zero-volume.md)

## Документи

- Довідка: [docs/help/uk/troubleshooting.md](../../help/uk/troubleshooting.md),
  [en](../../help/en/troubleshooting.md)
- Специфікація довідки: [help-content-polish](p1-help-content-polish.md)
  (мапа `troubleshooting.md`, жанр «відповідь без механіки», правило порядку,
  принцип універсального дизайну)
- Суміжні: [hotkeys-expansion](p2-hotkeys-expansion.md) (`Ctrl+M`, `F9`),
  [help-troubleshooting](p1-help-troubleshooting.md) (розділ як такий),
  [zero-volume-reads-as-muted](p2-zero-volume-reads-as-muted.md),
  [sound-hotkeys-feedback-announce-only](p1-sound-hotkeys-feedback-announce-only.md)
- Код: `src/lib/muteControl.ts` (дія й тексти стану), `src/stores/player.ts`
  (`$muteState`), `src/components/player/PlayerPanel.tsx` (кнопка — єдиний
  видимий індикатор стану)
