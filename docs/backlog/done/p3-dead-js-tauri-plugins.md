...
summary: "Tauri CLI звіряє major.minor кожного плагіна, на `tauri build` розбіжність = `Err`; пара без npm-сторони випадає з перевірки — ось підстава зняття."
priority: P3
type: planned
status: done
effort: S
...
Запис оцінював ціну як «час `pnpm install` і хибний сигнал» — і недооцінив її. **Головна ціна
була латентна:** Tauri CLI звіряє major.minor **кожного** відомого плагіна (крейт
`tauri-plugin-<p>` проти npm `@tauri-apps/plugin-<p>`), і відома всім перевірка `tauri` ↔
`@tauri-apps/api` — просто перший елемент того самого списку; на `tauri build` розбіжність
повертає **`Err`**, тобто збірка падає. Крейт `tauri-plugin-log` стоїть на 2.8.0 при
опублікованій 2.9.1, отже один самотній `cargo update` зробив би білд червоним помилкою без
стосунку до коду. Пара випадає з перевірки лише тоді, коли однієї зі сторін немає, — це і є
справжня підстава зняття, а не двадцять рядків локу. **Питання-стоп запису закрилось у
протилежний бік:** потреба «відкрити діалог із webview» не «може з'явитись», вона вже задоволена
**без** npm — `openDirectoryPicker()` кличе власну команду `open_directory_picker`, а команди
`plugin:dialog|open`/`|save`, які кличе npm-обгортка, всередині роблять **ті самі**
`blocking_pick_*`; JS-фасад іншого механізму не дає. Дозволи `dialog:default` / `log:default`
приходять із `permissions/` **усередині крейта** через `links` і `DEP_*_PERMISSION_FILES_PATH` —
npm у цьому ланцюзі не згадується жодного разу, і `gen/schemas/acl-manifests.json` несе текст
крейта дослівно. JS-бік логера потрібен рівно для `TargetKind::Webview`, якого в Tapir немає
(тільки `Folder` і `Stdout`). Для NVDA обидва шляхи сходяться в один `rfd::AsyncFileDialog`, тож
слухової різниці немає. Вимір: `-20` рядків локу, резолюція `@tauri-apps/api` **не змінилась**
(2.10.1 до і після), ворота зелені з першого прогону. Два хвости лишились свідомо:
`dialog:default`/`log:default` у capabilities мертві **за читанням**, але це окреме питання з
іншою ціною помилки; `tech-stack.md` §«Tauri Plugins» після зняття описує їх неточно.
Дослідження — [notes/dead-js-tauri-plugins.md](../../notes/dead-js-tauri-plugins.md).
