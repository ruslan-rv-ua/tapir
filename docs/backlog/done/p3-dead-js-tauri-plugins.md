...
Останній критерій **не закритий агентом** і лишається за людиною: обидві поверхні —
нативні (папка вибирається OS-діалогом, лог пише Rust), і жодна з них не керується з
сесії. Підстава закривати запис попри це — не «і так зрозуміло», а вимір: зняте не
брало участі ні в тому, ні в іншому шляху (див. нижче), а `vite:build` доводить, що
бандл, який отримує застосунок, зібрався без обох пакетів.
...
- **`dialog:default` / `log:default` у `capabilities/default.json`.** За читанням коду
  ...
  перевіркою.
- **`tech-stack.md` §«Tauri Plugins»** описує ці два плагіни як такі, що їх кличе
  webview. Після зняття формулювання неточне — але правка документа поза критеріями
  цього запису; хвіст для наступного проходу дрейфу документації.
