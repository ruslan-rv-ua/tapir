...
summary: "NVDA-прогін 4/4 за `excludeFromTabOrder` на транспорті плеєра: на слух різниці немає — `Tab` і раніше вів `usePlayerZoneNav`; коду в записі немає."
priority: P2
type: planned
status: done
effort: S
...
Ручний прогін під NVDA за зміною, яку [typecheck-gate](p1-typecheck-gate.md) зробив попутно:
п'ять кнопок транспорту плеєра пішли з рідного порядку `Tab` (`tabIndex={-1}` react-aria
ігнорував, тож усі стояли з `tabindex="0"` всупереч моделі зони; підтримуваний спосіб —
`excludeFromTabOrder`). Прогін **чистий, 4/4** — і підтвердив саме те, чого запис і чекав: **на
слух різниці немає**, бо `Tab` усередині панелі й до зміни йшов через
`usePlayerZoneNav.onRootKeyDown`, а не через рідний порядок; рідний порядок був неправильний
мовчки. Перевірено окремо, що `excludeFromTabOrder` не забрав кнопки в самої зони (стрілки
доводять до всіх п'яти, кожна називається), що вимкнена кнопка не стає безмовною пасткою, а
випадає з обходу, і що коротший набір стопів живого джерела ходить у тому ж порядку. Запис
існував тільки заради цього прогону: коду в ньому немає, тож і спадку, крім самого «перевірено»,
він не лишає — сама зміна описана в рядку typecheck-gate.
