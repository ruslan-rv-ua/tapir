...
title: "Подавити перезавантаження webview (F5 / Ctrl+R / контекстне меню Reload)"
priority: P2
type: planned
status: ready
effort: S
...
# Подавити перезавантаження webview (F5 / Ctrl+R / контекстне меню Reload)
...
1. **Клавіші:** `F5`, `Ctrl+F5`, `Shift+F5`, `Ctrl+R`, `Ctrl+Shift+R` —
   capture-фазний keydown-listener з `preventDefault()`. Метчити по `e.code`
   (`"F5"`, `"KeyR"`) — конвенція №1 [keyboard-shortcuts.md](../keyboard-shortcuts.md).
...
Ширший аудит акселераторів (Ctrl+P друк, Ctrl+F find тощо) — свідомо поза
скоупом: реального шкідливого ефекту, як у reload, вони не мають.

## Критерії готовності

- [ ] keydown-гард: `F5`/`Ctrl+F5`/`Shift+F5`/`Ctrl+R`/`Ctrl+Shift+R` →
      `defaultPrevented` (capture-фаза, працює і при відкритій модалі)
