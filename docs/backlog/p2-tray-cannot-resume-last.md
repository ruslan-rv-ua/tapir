...
summary: "огрилено 2026-09-24: трей продовжує останнє, як `Ctrl+Shift+K`; відмову у фоні несе тост фонового відгуку; панель — окремо в 0.2.0"
priority: P2
type: planned
status: ready
effort: M
...
- [ ] Знімок меню знає, чи записане останнє джерело; «нічого не грає» розпадається на два стани;
  `playback_items` і `from_status` лишаються чистими й вичерпними, новий стан названо в
  табличних тестах
- [ ] Активний пункт у спокої йде в `toggle_playback` — той самий вхід і debounce, що в `Ctrl+Shift+K`
- [ ] Меню перебудовується після `clear_last_session` і після підміни профілю в `switch_profile`
- [ ] `resume_last` поза фокусом показує тост фонового відгуку на «недоступне» й «помилку»;
  `player-announce` іде як раніше; «Підключення» й «Відтворення з позиції» тосту не мають
- [ ] `ToastKind::HotkeyFeedback` → `BackgroundFeedback` окремим комітом; `AGENTS.md`,
  `accessibility.md` і обидва ADR оновлено (§5)
- [ ] «cold» у значенні «не грає нічого» замінено окремим комітом (§9)
- [ ] ADR 2026-09-01 має рядок «продовжити останнє» в таблиці §4
- [ ] Доккоментар `handlers.rs` описує рівно ті шляхи, які з трея досяжні
- [ ] `docs/help/` оновлено в обох локалях: player.md і background.md (§7)
- [ ] NVDA-прогін за `docs/testing/nvda-tray-cannot-resume-last.json` чистий (§11)
- [ ] `cargo test`, `cargo clippy --all-targets`, `pnpm test`, `pnpm vite:build` зелені
