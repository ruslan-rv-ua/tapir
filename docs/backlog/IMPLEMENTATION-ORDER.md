...
`activity-bar-help-button`, `streams-ctrlk-empty-hint`).
...
|---|-------|---|-----|------|---------|--------------|----------------------|
| 1 | [wishlist-example-patterns](p2-wishlist-example-patterns.md) | P2 | заплановано | ready | S | — | патерн порожн. стану як у виконаному `streams-ctrlk-empty-hint` |
| 2 | [resume-last-playback](p2-resume-last-playback.md) | P2 | покращення | draft (модель ✅) | M | **надбудова над** playback-toggle ✅ (`PlayerSession`) | autostart ✅ |
| 3 | [log-rotation](p2-log-rotation.md) | P2 | ідея | draft* | S | — | — |
| 4 | [open-song-with-default-app](p2-open-song-with-default-app.md) | P2 | ідея | draft | S | Phase 3C | — |
| 5 | [import-duplicate-metadata-update](p2-import-duplicate-metadata-update.md) | P2 | ідея | draft | S | Phase 3J | — |
| 6 | [full-edit-stream](p2-full-edit-stream.md) | P2 | покращення | ідея | M | F2 edit-режим ✅ | — |
...
### Хвиля 1 — швидкі перемоги (P2/P3) + housekeeping

**1. [wishlist-example-patterns](p2-wishlist-example-patterns.md)** (P2, S, ready) — onboarding
для порожнього Wishlist/Ignorelist; усі питання закриті 2026-06-24. Той самий патерн
порожнього стану, що й у `streams-ctrlk-empty-hint` — тепер є реалізований зразок у
...
**2. [resume-last-playback](p2-resume-last-playback.md)** (P2, M, draft — модель ✅ узгоджена 2026-06-25)
...
**3. [log-rotation](p2-log-rotation.md)** · **4. [open-song-with-default-app](p2-open-song-with-default-app.md)** ·
**5. [import-duplicate-metadata-update](p2-import-duplicate-metadata-update.md)** ·
**6. [full-edit-stream](p2-full-edit-stream.md)** (M — потребує дизайну guard'а для активного запису).
...
1. ✅ **resume-last-playback (#2) vs playback-toggle** — _вирішено 2026-06-25 (A1), базу
   реалізовано 2026-07-18._ #2 — надбудова над `PlayerSession`; **окремого
...
2. ✅ **state.json (crash-recovery, done) vs стан відтворення (#2)** — _знято (A1)._ Стан
...
- **Готові до коду зараз** (`ready`/«рішення прийняті»): #1, #3, #14.
- **Модель узгоджена, база в коді** (промпт уже імплементаційний): #2 (A1 закрито 2026-06-25;
  `PlayerSession`-спадщина злита 2026-07-18, включно з `resume_file_from`).
- **Потребують grooming-пасу** (тип `ідея`/`draft`, є відкриті питання): #4, #5, #6,
  #7, #8, #9 — їхній промпт у записі досі «лише обговорення»; спершу довести до
  `заплановано`, тоді кодити.
- **Дослідження/розвилка**: #10, #11.
