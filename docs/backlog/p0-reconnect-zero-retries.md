...
- [ ] `plan_retry` — єдине джерело правила «пробувати ще раз?»; жодного
      `max_retries == 0` поза нею
- [ ] `ReconnectConfig::default().max_retries == 10`; кламп верхньої межі 10000 на
      бекенді, `maxValue` в UI
- [ ] `attempt = 0` — на першому отриманому аудіобайті, не на успішному `connect`
- [ ] Ключі `status_reconnecting_attempt_unlimited` (uk + en) і гілка в
      `StreamItem.tsx` видалені; при `maxRetries === 0` рядок показує
      `m.status_reconnecting()`
- [ ] `settings_max_retries_desc` (uk + en) каже «0 — не перепідключатися»
- [ ] Тести на `plan_retry` таблицею (нуль, стеля, backoff, кламп) — без `AppHandle`;
      заразом покрито `compute_backoff_delay`, який зараз голий
- [ ] `docs/help/uk/recording.md` і `docs/help/en/recording.md` — новий дефолт і як
      його вимкнути нулем
- [ ] `CONTEXT.md` — розділ «Перепідключення» (спроба, коли витрачена, що означає нуль)
- [ ] `docs/decisions/2026-08-13-reconnect-attempt-semantics.md` створено
...
- [help-recording](done/p1-help-recording.md) — запис, під час якого знайдено; містить звірені з кодом числа перепідключення
