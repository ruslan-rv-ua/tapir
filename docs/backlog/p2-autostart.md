# Autostart (Підфаза 3I-2)

- **Слаг:** `autostart`
- **Тип:** ідея
- **Стан:** draft
- **Зусилля:** S
- **Оновлено:** 2026-06-15
- **Залежності:** Phase 2C ✅ (SettingsDialog — додати нову опцію туди); Phase 3A ✅ (System Tray — запуск мінімізованим); Phase 3D ✅ (Scheduler — ключовий use case автостарту)

## Опис

Tapir має вміти запускатися автоматично разом із Windows, без участі користувача.

**Ключовий use case:** користувач налаштував запланований запис (Scheduler, фаза 3D) на ніч. Щоб запис відбувся, Tapir має бути запущений до моменту старту. Autostart дозволяє системі самостійно запустити застосунок при вході в Windows — навіть якщо користувач забув зробити це вручну.

Для незрячого користувача з NVDA це особливо важливо: він не може легко перевірити, чи застосунок запущений у фоні. Autostart знімає цю тривогу — треба лише одного разу увімкнути опцію.

### Технічне рішення

**Механізм:** `tauri-plugin-autostart` (офіційний плагін Tauri), який записує шлях до EXE у `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`. Реєстр `HKCU` (а не `HKLM`) — не вимагає прав адміністратора, що узгоджується з портативною моделлю Tapir.

**Команда запуску в реєстрі:** `tapir.exe --minimize` — щоб при autostart застосунок одразу згортався у System Tray без показу вікна. Аргумент `--minimize` вже є в Phase 3G (CLI Arguments).

**Налаштування:** новий toggle «Запускати разом із Windows» у SettingsDialog (Phase 2C). Поле у `Settings` — `autostart_enabled: bool`. При зміні toggle — негайний виклик `tauri_plugin_autostart::enable()` або `disable()`.

### Проблема Portable EXE

Tapir — portable застосунок, EXE можна перемістити в будь-яку папку. Якщо EXE переміщено після реєстрації autostart:
- Ключ реєстру вказує на старий (неіснуючий) шлях — Windows не запустить нічого, але й помилки не покаже.
- При наступному запуску з нового розташування Tapir **повинен** виявити цю розбіжність.

**Алгоритм перевірки при старті:**
1. Прочитати поточний `settings.autostart_enabled`.
2. Якщо `true` — через `tauri_plugin_autostart` отримати зареєстрований шлях і порівняти з `std::env::current_exe()`.
3. Якщо шляхи розходяться — **тихо деактивувати** autostart (`disable()`) і оновити `settings.autostart_enabled = false`.
4. Оголосити через NVDA (polite): «Autostart деактивовано: виявлено переміщення застосунку».

Деактивація тиха (не питати користувача) — це детерміновано коректна поведінка, не дія з побічними ефектами.

### Взаємодія з System Tray (3A ✅)

При autostart застосунок запускається з аргументом `--minimize`, тому вікно не з'являється — тільки іконка у треї. Після цього поведінка повністю стандартна (кліки по треї, відновлення вікна тощо).

### Взаємодія зі Scheduler (3D ✅)

Scheduler сам по собі не забезпечує запуск Tapir — він лише надсилає команди вже запущеному застосунку. Autostart закриває цю прогалину: при вході в систему Tapir запускається, ховається у трей, і Scheduler спрацьовує за розкладом вночі.

### Доступність (NVDA)

- Toggle «Запускати разом із Windows» у SettingsDialog — стандартний `<Switch>` з React Aria, повністю доступний.
- При увімкненні/вимкненні — polite-оголошення через `LiveAnnouncer`: «Autostart увімкнено» / «Autostart вимкнено».
- При виявленні переміщеного EXE на старті — polite NVDA-оголошення про деактивацію.
- Якщо запуск відбувся через autostart — за потреби можна додати polite-оголошення типу «Tapir запущений автоматично» (відкрите питання).

## Критерії готовності

- [ ] Toggle «Запускати разом із Windows» присутній у SettingsDialog і доступний з клавіатури/NVDA
- [ ] Увімкнення toggle реєструє EXE у `HKCU\...\Run` з параметром `--minimize`
- [ ] Вимкнення toggle видаляє запис з реєстру
- [ ] При старті перевіряється відповідність зареєстрованого шляху та `current_exe()`
- [ ] Якщо EXE переміщено — autostart тихо деактивується, `autostart_enabled` скидається в `false`
- [ ] При деактивації через переміщення — polite NVDA-оголошення
- [ ] Toggle у SettingsDialog відображає актуальний стан після перевірки при старті
- [ ] При autostart застосунок стартує з `--minimize` і з'являється лише у System Tray

## Відкриті питання

- **Мінімізація при autostart:** чи `--minimize` має бути захардкоджено у команді реєстру, чи це має бути окреме налаштування «Запускати мінімізованим»? Якщо окреме — де воно (поруч з toggle або в секції Tray)?
- **Деактивація при переміщенні — тихо чи питати?** Поточне рішення — тихо з NVDA-оголошенням. Альтернатива — спитати через діалог. Для незрячого користувача тихий підхід безпечніший (менше несподіваних діалогів при старті). Схиляюся до тихого.
- **Оголошення при autostart-старті:** чи оголошувати polite «Tapir запущений автоматично» при кожному autostart? Може бути корисним для орієнтації, але також дратівливим при кожному вході в систему.
- **Phase 3G залежність:** `--minimize` реалізується в Phase 3G (CLI Arguments). Якщо 3G буде після 3I-2 — або реалізувати `--minimize` окремо в межах цієї задачі, або зробити autostart без мінімізації до реалізації 3G.

## Документи

- [implementation-phases.md §3I](../implementation-phases.md) — підфаза 3I-2 (Autostart)
- [implementation-phases.md §3A](../implementation-phases.md) — System Tray (залежність)
- [implementation-phases.md §3D](../implementation-phases.md) — Scheduler (ключовий use case)
- [p1-cli-arguments.md](p1-cli-arguments.md) — аргумент `--minimize` (потрібен для autostart)
- Код: `src-tauri/src/settings.rs` — структура `Settings` (додати поле `autostart_enabled`)
- Код: `src-tauri/src/lib.rs` — перевірка шляху EXE при старті
- Код: `src/components/settings/` — SettingsDialog (додати toggle)
- Код: `src/i18n/messages/uk.json`, `en.json` — i18n-ключі для нових рядків

## Промпт для агента

Каталог промптів за типом: [README — Каталог промптів](README.md#каталог-промптів-за-типом).
Тип `ідея`.
