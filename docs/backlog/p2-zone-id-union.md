...
notes:
  ...
  - "Паралельна гілка chore/zone-proxy-hook уже несе useZoneProxy(id: string, …): хто зливається другим, міняє там string на ZoneId — одне слово."
---
...
- **Порядок із [zone-proxy-hook](p2-zone-proxy-hook.md).** Домовленість була «союз
  першим», але на момент грумінгу гілка `chore/zone-proxy-hook` уже мала готовий коміт
  із `useZoneProxy(id: string, …)`. Хто зливається другим, міняє там `string` на
  `ZoneId` — одне слово, і компілятор сам на нього вкаже.
...
- [zone-proxy-hook](p2-zone-proxy-hook.md) — сусідній хвіст у тому самому файлі
