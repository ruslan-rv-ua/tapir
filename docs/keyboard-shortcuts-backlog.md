...
- **Уточнено (KB-14, 2026-06-08):** текстовий блок прибрано з глобального слухача —
  глушиться лише модаль. Колізійна частина гарда стосувалась лише немодифікованих
  клавіш, яких Tier-2 не має.
...
### [x] KB-12 · ✨ Можливі нові глобальні шорткати
...
([App.tsx](../src/App.tsx)) глушився **текстовим полем АБО модаллю**, тоді як
`F6` зон-навігація ([useZoneNavigation.ts](../src/hooks/useZoneNavigation.ts)) і
клавіші рядка ([useCompositeList.ts](../src/hooks/useCompositeList.ts)) — **лише
модаллю**. Через це `Alt+2`/`Ctrl+K`/`F1` не працювали з фокусом у пошуку Browser,
хоча `F6` працював. Плюс `useCompositeList` повторював селектор модалі інлайн-
літералом, який **відстав** від канонічного `MODAL_SELECTOR` (бракувало
`[aria-modal="true"]`).
- **Готово коли:** глобальні шари мають єдину політику подавлення; `Alt+digit`
  тощо працюють із текстового поля, але не поверх модалі; селектор модалі — один.
- **Зроблено (2026-06-08):** концептуально розділено дві причини глушити —
  *контекст* (модаль/рекордер; універсальний `isInModal`) і *колізію* (набір
  тексту; стосується лише немодифікованих клавіш). Усі Tier-2 комбо модифіковані
  або `F1`, тож колізії не дають → слухач [App.tsx](../src/App.tsx) гейтиться
  **лише на `isInModal()`**. `shouldIgnoreShortcut` прибрано (став зайвим);
  `isTextEntryTarget` лишено для майбутніх клавіш рядка Tier 2′. `useCompositeList`
